...
class ReadConfigSchema(BaseModel):
    node_proto_id: int
    flatten_json_users_key: list[str] = Field(
        default=None,
        description="Ключ к списку пользователей в конфиге. При чтении с админки этот объект вырезается во избежание лишних сетевых расходов",
    )
...
class WriteConfigSchema(BaseModel):
    node_proto_id: int
    file_content: str
    flatten_json_users_key: list[str] | None = Field(
        None, description='Ключ к списку пользователей в конфиге. При записи этот объект переносится из старого файла'
    )
...
class UserCoreProtoActionSchema(BaseModel):
    uuid: str = Field(
        max_length=36,
        description='Буквенный ID пользователя, обычно используется в списках пользователей в ядрах протоколов',
    )
