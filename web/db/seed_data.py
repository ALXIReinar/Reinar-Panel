...
async def insert_with_fixed_id(conn: Connection, table: str, records: list[dict[str, Any]]) -> None:
    """
...
async def insert_adaptive(
    conn: Connection, table: str, records: list[dict[str, Any]], unique_fields: list[str]
) -> None:
    """
...
async def insert_proto_templates(conn: Connection, templates: list[dict[str, Any]]) -> None:
    """
    ...
    for template_data in templates:
        ...
        for extractor in extractors:
            ...
            exists = await conn.fetchval(
                ...
                extractor_data["flatten_array_cursor"],
                template_id,
            )
            if not exists:
    ...
    print(
        f"✓ proto_templates: {inserted_templates} шаблонов, {inserted_params} параметров, {inserted_protocols} протоколов, {inserted_extractors} экстракторов"
    )
...
async def seed_all(conn: Connection, seed_data: dict[str, Any]) -> None:
    ...
    print("\n2. Статусы платежей...")
    await insert_with_fixed_id(conn, "pay_statuses", seed_data.get("pay_statuses", []))
    print("\n3. Статусы онлайн...")
    await insert_with_fixed_id(conn, "online_statuses", seed_data.get("online_statuses", []))
    print("\n4. Операции с нодами подписок...")
    await insert_with_fixed_id(conn, "sub_nodes_operations", seed_data.get("sub_nodes_operations", []))
    print("\n5. Белый список команд...")
    await insert_adaptive(conn, "whitelist_commands", seed_data.get("whitelist_commands", []), ["command"])
    # 2. Сложные таблицы с FK и вложенными данными
    print("\n6. Шаблоны протоколов (с вложенными данными)...")
    await insert_proto_templates(conn, seed_data.get("proto_templates", []))
