...
async def users_with_subs(db_pool, virtual_node_seed, sub_plan_seed):
    ...
    async with db_pool.acquire() as conn:
        ...
        traffic_order_id = await conn.execute(  # noqa: F841
            """
            INSERT INTO user_subs (
                user_id, order_id, sub_plan_id, is_active, is_limited,
...
class TestBulkUpdateActivate:
    ...
    async def test_activate_success(self, client, users_with_subs, mock_arq, db_pool):
        ...
        response = await client.put(
            "/api/v1/private/users/bulk/update", json={"user_ids": user_ids, "action": "activate"}
        )
        ...
        async with db_pool.acquire() as conn:
            activated = await conn.fetch(
                "SELECT id, is_active FROM user_subs WHERE user_id = ANY($1) ORDER BY id", user_ids
            )
...
class TestBulkUpdateDeactivate:
    ...
    async def test_deactivate_success(self, client, users_with_subs, mock_arq, db_pool):
        ...
        response = await client.put(
            "/api/v1/private/users/bulk/update", json={"user_ids": user_ids, "action": "deactivate"}
        )
...
class TestBulkUpdateResetTraffic:
    ...
    async def test_reset_traffic_success(self, client, users_with_subs, mock_arq, db_pool):
        ...
        response = await client.put(
            "/api/v1/private/users/bulk/update", json={"user_ids": [user_id], "action": "reset_traffic"}
        )
        ...
        async with db_pool.acquire() as conn:
            traffic = await conn.fetchval("SELECT traffic_used_day_mb FROM user_subs WHERE user_id = $1", user_id)
            assert traffic == 0
...
class TestBulkUpdateValidation:
    ...
    async def test_invalid_action(self, client, users_with_subs, mock_arq):
        ...
        response = await client.put(
            "/api/v1/private/users/bulk/update", json={"user_ids": [1, 2], "action": "invalid_action"}
        )
    ...
    async def test_empty_user_ids(self, client, mock_arq):
        ...
        response = await client.put("/api/v1/private/users/bulk/update", json={"user_ids": [], "action": "activate"})
    ...
    async def test_nonexistent_user_ids(self, client, mock_arq):
        ...
        response = await client.put(
            "/api/v1/private/users/bulk/update", json={"user_ids": [99999, 88888], "action": "activate"}
        )
...
class TestBulkUpdateEdgeCases:
    ...
    @pytest.mark.asyncio
    async def test_activate_skips_limited_subscriptions(
        self, client, db_pool, virtual_node_seed, sub_plan_seed, mock_arq
    ):
        ...
        async with db_pool.acquire() as conn:
            ...
            user_id = await conn.fetchval(
                "INSERT INTO users (tg_id, tg_username) VALUES ($1, $2) RETURNING id", 2000001, "limited_user"
            )
        ...
        response = await client.put(
            "/api/v1/private/users/bulk/update", json={"user_ids": [user_id], "action": "activate"}
        )
        ...
        async with db_pool.acquire() as conn:
            ...
            is_active = await conn.fetchval("SELECT is_active FROM user_subs WHERE id = $1", user_sub_id)
            assert is_active is False

    @pytest.mark.asyncio
    async def test_deactivate_skips_limited_subscriptions(
        self, client, db_pool, virtual_node_seed, sub_plan_seed, mock_arq
    ):
        ...
        async with db_pool.acquire() as conn:
            ...
            user_id = await conn.fetchval(
                "INSERT INTO users (tg_id, tg_username) VALUES ($1, $2) RETURNING id", 2000002, "limited_active_user"
            )
        ...
        response = await client.put(
            "/api/v1/private/users/bulk/update", json={"user_ids": [user_id], "action": "deactivate"}
        )
    ...
    @pytest.mark.asyncio
    async def test_reset_traffic_resets_all_but_outbox_only_limited(
        self, client, db_pool, virtual_node_seed, sub_plan_seed, mock_arq
    ):
        ...
        async with db_pool.acquire() as conn:
            ...
            user_id_limited = await conn.fetchval(
                "INSERT INTO users (tg_id, tg_username) VALUES ($1, $2) RETURNING id", 2000003, "limited_traffic_user"
            )
            user_id_normal = await conn.fetchval(
                "INSERT INTO users (tg_id, tg_username) VALUES ($1, $2) RETURNING id", 2000004, "normal_traffic_user"
            )
        ...
        response = await client.put(
            ...
            json={"user_ids": [user_id_limited, user_id_normal], "action": "reset_traffic"},
        )
        ...
        async with db_pool.acquire() as conn:
            traffic_limited = await conn.fetchval(
                "SELECT traffic_used_day_mb FROM user_subs WHERE user_id = $1", user_id_limited
            )
            traffic_normal = await conn.fetchval(
                "SELECT traffic_used_day_mb FROM user_subs WHERE user_id = $1", user_id_normal
            )
            ...
            outbox_limited = await conn.fetchval(
                "SELECT COUNT(*) FROM sub_nodes_outbox WHERE user_sub_id = $1", order_id_limited
            )
            outbox_normal = await conn.fetchval(
                "SELECT COUNT(*) FROM sub_nodes_outbox WHERE user_sub_id = $1", order_id_normal
            )
            assert outbox_limited == 1  # limited пользователь в outbox
            assert outbox_normal == 0  # normal пользователь НЕ в outbox

    @pytest.mark.asyncio
    async def test_user_with_multiple_subscriptions_activate_only_inactive(
        self, client, db_pool, virtual_node_seed, sub_plan_seed, mock_arq
    ):
        ...
        async with db_pool.acquire() as conn:
            ...
            user_id = await conn.fetchval(
                "INSERT INTO users (tg_id, tg_username) VALUES ($1, $2) RETURNING id", 2000005, "multi_sub_user"
            )
        ...
        async with db_pool.acquire() as conn:
            active_sub_status = await conn.fetchval("SELECT is_active FROM user_subs WHERE id = $1", order_id_active)
            inactive_sub_status = await conn.fetchval(
                "SELECT is_active FROM user_subs WHERE id = $1", order_id_inactive
            )
            ...
            outbox_active = await conn.fetchval(
                "SELECT COUNT(*) FROM sub_nodes_outbox WHERE user_sub_id = $1", order_id_active
            )
            outbox_inactive = await conn.fetchval(
                "SELECT COUNT(*) FROM sub_nodes_outbox WHERE user_sub_id = $1", order_id_inactive
            )
    ...
    async def test_invisible_vnode_not_in_outbox(self, client, db_pool, virtual_node_seed, sub_plan_seed, mock_arq):
        ...
        async with db_pool.acquire() as conn:
            ...
            # Создаём пользователя
            user_id = await conn.fetchval(
                "INSERT INTO users (tg_id, tg_username) VALUES ($1, $2) RETURNING id", 2000006, "invisible_vnode_user"
            )

            # Привязываем невидимую vnode к плану
            plan_id = sub_plan_seed["plan_id_1"]
            await conn.execute(
                "INSERT INTO vnodes_sub_plans (node_proto_id, sub_plan_id) VALUES ($1, $2)", invisible_vnode_id, plan_id
            )
    ...
    @pytest.mark.asyncio
    async def test_inactive_physical_node_not_in_outbox(
        self, client, db_pool, virtual_node_seed, sub_plan_seed, mock_arq
    ):
        ...
        async with db_pool.acquire() as conn:
            ...
            # Создаём пользователя
            user_id = await conn.fetchval(
                "INSERT INTO users (tg_id, tg_username) VALUES ($1, $2) RETURNING id", 2000007, "inactive_node_user"
            )

            # Привязываем vnode к плану
            plan_id = sub_plan_seed["plan_id_1"]
            await conn.execute(
                "INSERT INTO vnodes_sub_plans (node_proto_id, sub_plan_id) VALUES ($1, $2)", vnode_id, plan_id
            )
