...
class TestBulkDeleteSuccess:
    ...
    async def test_bulk_delete_removes_users(self, client, users_with_subs_for_delete, mock_arq, db_pool):
        ...
        response = await client.request("DELETE", "/api/v1/private/users/bulk/delete", json={"user_ids": user_ids})
        ...
        async with db_pool.acquire() as conn:
            users = await conn.fetch("SELECT id, is_deleted FROM users WHERE id = ANY($1)", user_ids)
    ...
    async def test_bulk_delete_deactivates_subscriptions(self, client, users_with_subs_for_delete, mock_arq, db_pool):
        ...
        async with db_pool.acquire() as conn:
            active_before = await conn.fetch("SELECT id, is_active FROM user_subs WHERE user_id = ANY($1)", user_ids)
        ...
        async with db_pool.acquire() as conn:
            subs_after = await conn.fetch("SELECT id, is_active FROM user_subs WHERE user_id = ANY($1)", user_ids)
...
class TestBulkDeleteOutboxAndArq:
    ...
    @pytest.mark.asyncio
    async def test_bulk_delete_creates_outbox_for_active_subs(
        self, client, users_with_subs_for_delete, mock_arq, db_pool
    ):
    ...
    @pytest.mark.asyncio
    async def test_bulk_delete_only_is_limited_false_in_outbox(
        self, client, users_with_subs_for_delete, mock_arq, db_pool
    ):
        ...
        response = await client.request("DELETE", "/api/v1/private/users/bulk/delete", json={"user_ids": all_user_ids})
    ...
    async def test_bulk_delete_empty_list_no_arq_call(self, client, mock_arq):
        ...
        response = await client.request("DELETE", "/api/v1/private/users/bulk/delete", json={"user_ids": []})
...
class TestBulkDeleteEdgeCases:
    ...
    async def test_bulk_delete_nonexistent_users(self, client, mock_arq):
        ...
        response = await client.request(
            "DELETE", "/api/v1/private/users/bulk/delete", json={"user_ids": [99999, 88888]}
        )
    ...
    @pytest.mark.asyncio
    async def test_bulk_delete_inactive_subscription_not_in_outbox(
        self, client, users_with_subs_for_delete, mock_arq, db_pool
    ):
        ...
        response = await client.request(
            "DELETE", "/api/v1/private/users/bulk/delete", json={"user_ids": [inactive_user_id]}
        )
        ...
        async with db_pool.acquire() as conn:
            is_deleted = await conn.fetchval("SELECT is_deleted FROM users WHERE id = $1", inactive_user_id)
    ...
    @pytest.mark.asyncio
    async def test_bulk_delete_is_limited_user_not_in_outbox(
        self, client, users_with_subs_for_delete, mock_arq, db_pool
    ):
        ...
        response = await client.request(
            "DELETE", "/api/v1/private/users/bulk/delete", json={"user_ids": [limited_user_id]}
        )
        ...
        # Проверяем что пользователь помечен как удалённый (soft delete)
        async with db_pool.acquire() as conn:
            is_deleted = await conn.fetchval("SELECT is_deleted FROM users WHERE id = $1", limited_user_id)
            assert is_deleted is True
    ...
    @pytest.mark.asyncio
    async def test_bulk_delete_invisible_vnode_no_outbox(
        self, client, virtual_node_seed, sub_plan_seed, mock_arq, db_pool
    ):
        ...
        async with db_pool.acquire() as conn:
            ...
            user_id = await conn.fetchval(
                "INSERT INTO users (tg_id, tg_username) VALUES ($1, $2) RETURNING id", 4001000, "invisible_delete_user"
            )
        ...
        response = await client.request("DELETE", "/api/v1/private/users/bulk/delete", json={"user_ids": [user_id]})
        ...
        # Проверяем что пользователь помечен как удалённый (soft delete)
        async with db_pool.acquire() as conn:
            is_deleted = await conn.fetchval("SELECT is_deleted FROM users WHERE id = $1", user_id)
            assert is_deleted is True
    ...
    @pytest.mark.asyncio
    async def test_bulk_delete_inactive_node_no_outbox(
        self, client, virtual_node_seed, sub_plan_seed, mock_arq, db_pool
    ):
