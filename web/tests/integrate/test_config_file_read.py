...
class TestConfigFileReadSuccess:
    ...
    async def test_read_config_success(self, client, virtual_node_seed, db_pool):
        ...
        client.app.state.cmd_center_aiohttp = FakeAiohttpSession(
            json_data={"content": '{"log": {"loglevel": "warning"}, "inbounds": []}'}
        )

        response = await client.get("/api/v1/private/cmd_center/config_file/read", params={"node_proto_id": vnode_id})
    ...
    async def test_read_config_with_flatten_key(self, client, virtual_node_seed, db_pool):
        ...
        async with db_pool.acquire() as conn:
            await conn.execute(
                "UPDATE nodes_protocols SET config_path = $1 WHERE id = $2", "/etc/v2ray/config.json", vnode_id
            )

        # Мокируем ответ с пользователями (которые должны быть вырезаны)
        client.app.state.cmd_center_aiohttp = FakeAiohttpSession(
            json_data={"content": '{"log": {"loglevel": "info"}, "clients": []}'}
        )

        response = await client.get(
            "/api/v1/private/cmd_center/config_file/read",
            params={"node_proto_id": vnode_id, "flatten_json_users_key": "inbounds.0.settings.clients"},
        )
...
class TestConfigFileReadErrors:
    ...
    async def test_read_config_node_error(self, client, virtual_node_seed, db_pool):
        ...
        async with db_pool.acquire() as conn:
            await conn.execute(
                "UPDATE nodes_protocols SET config_path = $1 WHERE id = $2", "/nonexistent/config.json", vnode_id
            )

        # Мокируем ответ с ошибкой 404 от ноды
        client.app.state.cmd_center_aiohttp = FakeAiohttpSession(json_data={"error": "File not found"}, status=404)
...
class TestConfigFileReadValidation:
    ...
    async def test_read_config_missing_node_proto_id(self, client):
        ...
        response = await client.get("/api/v1/private/cmd_center/config_file/read", params={})
