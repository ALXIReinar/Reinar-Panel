...
# Генерация служебного UUID, по которому Entry-ноды будут подключаться к Exit-ноде
...
[Unit]
Description=Xray Exit Node VMess-TCP-TLS (TMP_ID: ${TMP_ID})
...
curl -s -X POST "$PANEL_CALLBACK_URL" \
     ...
     -d '{
           ...
           "status": "installed",
           "node_type": "exit_vmess_tcp_tls"
         }'

echo "VMess Exit-нода $TMP_ID установлена. Служебный UUID: $EXIT_UUID"