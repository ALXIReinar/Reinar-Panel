...
class TestRevokeSubPlanByExpire:
    ...
    async def test_revoke_sql_filters_critical(self, arq_ctx, arq_pool, revoke_seed, db_pool):
        ...
        async with db_pool.acquire() as conn:
            ...
            user_c_sub = next(
                s for s in deactivated_subs if s['user_id'] == seed['should_not_revoke']['user_c']['user_id']
            )
            ...
            user_d_sub = next(
                s for s in deactivated_subs if s['user_id'] == seed['should_not_revoke']['user_d']['user_id']
            )
    ...
    async def test_revoke_enqueues_bulk_delete_jobs(self, arq_ctx, arq_pool, revoke_seed, db_pool):
        ...
        async with db_pool.acquire() as conn:
            nodes_count = await conn.fetchval(
                ...
                JOIN nodes_protocols np ON np.id = vsp.node_proto_id AND np.user_visible = true AND np.reg_status = $2
