...
def subs_intro_kb():
    ...
    kb.button(text='🔄 Продлить', callback_data='subs-upd')
    kb.button(text='➕ Купить новую', callback_data=f'subs-buy')
...
def user_sub_plan_offers(user_sub_idx, costs_days_list):
    ...
    """
    kb = InlineKeyboardBuilder()
    emojis = env.emoji_period_offers

    for offer_idx, item in enumerate(costs_days_list):
        cost, days = item['cost'], item['ttl_days']
        kb.button(
            text=f'{random.choice(emojis)} {cost}₽ / {days} дней',
            callback_data=f'subs-user-offer_{user_sub_idx}_{offer_idx}',
        )
...
def shop_sub_plan_offers(sub_plan_idx, costs_days_list):
    ...
    for offer_idx, item in enumerate(costs_days_list):
        ...
        kb.button(
            text=f'{random.choice(emojis)} {cost}₽ / {days} дней',
            callback_data=f'subs-shop-offer_{sub_plan_idx}_{offer_idx}',
        )
