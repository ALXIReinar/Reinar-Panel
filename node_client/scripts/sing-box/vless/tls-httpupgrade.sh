...
if [ -z "$CERT_PATH" ] || [ -z "$KEY_PATH" ] || [ -z "$DOMAIN" ]; then
    ...
    log "Использование: bash node_client/scripts/sing-box/vless/tls-httpupgrade.sh <cert_path> <key_path> <domain>"
    exit 1
fi
...
CONFIG_DIR="/etc/reinar/configs/sing-box/vless"
...
og "Выделен внутренний порт для Sing-box: $INTERNAL_PORT"
...
CONFIG_PATH="$CONFIG_DIR/vless-tls-httpup-${NODE_PROTO_ID}.json"
...
[Unit]
Description=Sing-box Vless HttpUpgrade TLS Node (NODE_PROTO_ID: ${NODE_PROTO_ID})
