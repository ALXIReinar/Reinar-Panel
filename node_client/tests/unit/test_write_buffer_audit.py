...
from node_client.tests.utils.test_data_factory import create_superuser_object  # noqa: F401
...
def create_vless_superuser(
    ...
    if user_sub_id is None:
        user_sub_id = 1
    return {"user_uuid": user_uuid, "user_sub_id": user_sub_id, "flow": flow, "level": level}
...
async def registered_vless_node(buffer, tmp_path, vless_extractor):
    ...
    config = {
        ...
        "inbounds": [{"port": 443, "protocol": "vless", "settings": {"clients": cutlets, "decryption": "none"}}],
    }
...
async def registered_shadowsocks_node(buffer, tmp_path, shadowsocks_extractor):
    ...
    superusers = [create_shadowsocks_superuser(user_uuid=str(uuid_lib.uuid4()), user_sub_id=i) for i in range(1, 3)]
    # Трансформируем в котлеты
        ...
        "inbounds": [
            {
                ...
                "settings": {"method": "2022-blake3-aes-256-gcm", "clients": cutlets},
            }
        ]
    }
    # Создаём state
...
async def test_medium_audit_length_mismatch_continues(buffer, registered_vless_node, caplog):
    ...
    config = orjson.loads(config_path.read_bytes())
    extra_client = {"id": str(uuid_lib.uuid4()), "email": 999, "flow": "xtls-rprx-vision", "level": 0}
...
async def test_strict_audit_content_mismatch_raises_exception(buffer, registered_vless_node):
    ...
    config = orjson.loads(config_path.read_bytes())
    alien_clients = [
        {"id": str(uuid_lib.uuid4()), "email": 999000 + i, "flow": "xtls-rprx-vision", "level": 0}
...
async def test_audit_multiple_injectors_all_pass(buffer, tmp_path, vless_extractor):
    ...
    superusers = [create_vless_superuser(user_uuid=str(uuid_lib.uuid4()), user_sub_id=i) for i in range(1, 4)]
    # Котлеты
    cutlets = [vless_extractor(u) for u in superusers]
...
async def test_audit_multiple_injectors_one_fails(buffer, tmp_path, vless_extractor, caplog):
    ...
    superusers = [create_vless_superuser(user_uuid=str(uuid_lib.uuid4()), user_sub_id=i) for i in range(1, 3)]
    cutlets = [vless_extractor(u) for u in superusers]
