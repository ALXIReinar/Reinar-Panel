...
class UpdateTmpSchema(BaseModel):
    ...
    proto_python_lib: str | None = Field(None, max_length=512, description='Библиотека для hot-reload (grpcio, requests)')
    sub_prepare_script: str | None = Field(None, description='Скрипт подготовки подписки')
    sub_required_libs: list[str] | str | None = Field(None, max_length=512, description='Требуемые библиотеки для подписки')
