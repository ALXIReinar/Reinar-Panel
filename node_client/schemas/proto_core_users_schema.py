...
class BaseUserCoreSchema(BaseModel):
    ...
    custom_params: dict | None = Field(
        description='Зависимости для скрипта, которые идут отдельно от объекта пользователя'
    )
    user_injectors: list["UserInjector"]
    users: list[dict]
    action_script: str | None
    config2json_script: str | None = Field(
        description='Конвертер-скрпт конфига из его формата в json-структуру(python dict)'
    )
    ...
    action: Literal["add", "delete"] = Field(
        description="Операция, выполняемая скриптом. Вставка или удаление. 1 - add, 2 - delete. Допускаются строки и цифры"
    )
...
class UserInjector(BaseModel):
    ...
    extractor_script: str = Field(
        description='скрипт-обработчик для трансформации user_obj под требования массива под flatten_array_cursor'
    )
    libs: str | None = Field(
        None,
        max_length=512,
        description='Либы, нужные для скрипта-экстрактора объекта пользователя для впн-ядра из Суперобъекта',
    )
