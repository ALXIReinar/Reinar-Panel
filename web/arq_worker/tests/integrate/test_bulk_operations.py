...
class TestBulkActionUsersAdd:
    ...
    async def _load_node_data_from_db(self, db_pool, arq_test_seed, operation=1):
        ...
        user_data = (
            arq_test_seed['user3_active_for_add'] if operation == 1 else arq_test_seed['user4_active_for_delete']
        )
        ...
        if not event_id:
            raise RuntimeError(
                f"Outbox не содержит запись для user_sub_id={user_data['user_sub_id']}, operation={operation}"
            )
    ...
    async def test_bulk_add_success(self, mock_arq_ctx, arq_test_seed, db_pool):
        ...
        async with db_pool.acquire() as conn:
            event_exists = await conn.fetchval("SELECT EXISTS(SELECT 1 FROM sub_nodes_outbox WHERE id = $1)", event_id)
