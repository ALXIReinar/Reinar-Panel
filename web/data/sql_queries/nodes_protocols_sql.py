...
class NodesProtocolsQueries:
    ...
    async def create_node_protocol(self, node_id: int, proto_id: int, title: str) -> int:
        """Добавить протокол на ноду"""
        query = "INSERT INTO nodes_protocols (node_id, proto_id, title) VALUES ($1, $2, $3) ON CONFLICT DO NOTHING RETURNING id"
        return await self.conn.fetchval(query, node_id, proto_id, title)
    ...
    async def get_node_protocol(self, np_id: int):
        ...
        SELECT
            ...
            np.proto_port, np.metrics_port, n.is_active, np.user_visible, np.title, np.config_link, np.config_path, n.title as node_title,
    ...
    async def get_node_protocols(self, node_id: int):
        ...
        SELECT 
            np.node_id, np.proto_id, p.name as proto_name, n.ip as node_ip, n.private_ip as node_private_ip, n.api_port as node_api_port,
            np.proto_port, np.metrics_port, n.is_active, np.user_visible, np.title, np.created_at, np.updated_at
        FROM nodes_protocols np
        JOIN protocols p ON np.proto_id = p.id
        JOIN nodes n ON np.node_id = n.id
        WHERE np.node_id = $1
    ...
    async def get_proto_tmp_w_spec_params(self, node_proto_id: int) -> tuple:
        tmp_link_query = '''
        SELECT pt.url_tmp, pt.separator, np.title FROM proto_templates pt
        ...
        config_link_tmp, sep, node_title = await self.conn.fetchrow(tmp_link_query, node_proto_id)
        spec_params = await self.conn.fetch(query_spec_params, node_proto_id)
        return config_link_tmp, sep, spec_params, node_title
