...
from node_client.tests.schemas.extractor_schemas import get_schema_for_template, get_expected_type_for_cursor
from node_client.tests.utils.extractor_helpers import generate_constant_node_data_obj_for_extractor
...
async def test_extractor_scripts_execution(protocol_templates_with_extractors):
    ...
    for template in protocol_templates_with_extractors:
        ...
        for extractor in extractors:
            ...
            try:
                ...
                constant_node_data_obj = generate_constant_node_data_obj_for_extractor(extractor['extractor_script'])
                ...
                transform_func = HotReloadExecutor.get_compiled_func(
                    func_script=extractor['extractor_script'], func_name='transform', libs=extractor.get('libs')
                )
                ...
                if expected_type == 'string':
                    ...
                    if not isinstance(result, (str, int)):
                        raise TypeError(f"Expected str or int for cursor '{cursor}', got {type(result).__name__}")
                    results.append(
                        {
                    ...
                    if not isinstance(result, dict):
                        raise TypeError(f"Expected dict for cursor '{cursor}', got {type(result).__name__}")
                    # Пытаемся получить схему. Если её нет - пропускаем валидацию
                    try:
                        schema = get_schema_for_template(template['title'])
                        jsonschema.validate(result, schema)
                    except ValueError as schema_error:  # noqa: F841
    ...
    assert len(errors) == 0, f"\n{len(errors)} extractors failed validation. See detailed report above."
