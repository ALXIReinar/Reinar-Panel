...
async def sub(db: PgSqlDep, request: Request, response: Response, b64_id: str = Field(min_length=16, max_length=64)):
    sub_meta, config_data = await db.get_sub_links(b64_id)
    ...
    response.headers["profile-update-interval"] = env.subscription_update_interval # Обновлять каждые 12 часов
