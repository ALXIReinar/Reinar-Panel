...
if [ -z "$CERT_PATH" ] || [ -z "$KEY_PATH" ] || [ -z "$DOMAIN" ]; then
    log "Ошибка: Не указан CERT_PATH, KEY_PATH, DOMAIN!"
    log "Использование: bash ss-ws-install.sh <cert_path> <key_path> <domain>"
    exit 1
fi
...
CONFIG_DIR="/etc/reinar/configs/xray/vmess"
...
GRPC_SERVICE_NAME="vmess-grpc-${TMP_ID}"
...
CONFIG_PATH="$CONFIG_DIR/vmess-tls-grpc-${NODE_PROTO_ID}.json"
cat <<EOF > "$CONFIG_PATH"
{
...
[Unit]
Description=Xray VMess-gRPC-TLS Node (NODE_PROTO_ID: ${NODE_PROTO_ID})
