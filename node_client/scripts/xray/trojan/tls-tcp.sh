...
if [ -z "$CERT_PATH" ] || [ -z "$KEY_PATH" ] || [ -z "$DOMAIN" ]; then
    ...
    log "Использование: bash trojan-tcp-tls-install.sh <cert_path> <key_path> <domain>"
    exit 1
fi
...
CONFIG_PATH="$CONFIG_DIR/trojan-tls-tcp-${NODE_PROTO_ID}.json"
cat <<EOF > "$CONFIG_PATH"
{
...
[Unit]
Description=Xray Trojan-TLS-TCP Node (NODE_PROTO_ID: ${NODE_PROTO_ID})
