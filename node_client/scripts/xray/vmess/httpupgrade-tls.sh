...
if [ -z "$TMP_ID" ] || [ -z "$DOMAIN" ] || [ -z "$CERT_PATH" ] || [ -z "$KEY_PATH" ]; then
    ...
    echo "Использование: bash vmess-httpupgrade-tls.sh <tmp_id> <domain> <cert_path> <key_path>"
    exit 1
fi
...
[Unit]
Description=Xray VMess-HTTPUpgrade-TLS Node (TMP_ID: ${TMP_ID})
...
curl -s -X POST "$PANEL_CALLBACK_URL" \
     ...
     -d '{
           ...
           "status": "installed",
           "node_type": "vmess_httpupgrade_tls",
         }'

echo "VMess-HTTPUpgrade-TLS нода $TMP_ID успешно установлена."