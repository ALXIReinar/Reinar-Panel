from typing import Literal, Annotated  # noqa: I001
...
class UpdateTmpSchema(BaseModel):
    ...
    proto_python_lib: str | None | int = Field(
        0, max_length=512, description='Библиотека для hot-reload (grpcio, requests)'
    )
    sub_prepare_script: str | None | int = Field(0, description='Скрипт подготовки подписки')
    sub_required_libs: list[str] | str | None | int = Field(
        0, max_length=512, description='Требуемые библиотеки для подписки'
    )
    api_bulk_delete_user_script: str | None | int = Field(
        0, description='Python скрипт для bulk удаления пользователей'
    )
    ...
    bulk_delete_script_custom_params: dict | None = Field(
        None, description='Кастомные параметры для bulk delete скрипта'
    )
    ...
    config2json_script: str | None | int = Field(
        0, description='Конвертер-скрпт конфига из его формата в json-структуру(python dict)'
    )
...
class EditUserInjectorsSchema(BaseModel):
    user_injectors: list["UserInjector"] = Field(
        description='State инжекторов шаблона. Передавать Итоговое состояние ВСЕХ инжекторов, если произошло хотя бы одно изменение'
    )
...
class UserInjector(BaseModel):
    ...
    def extractor_script_validator(cls, v):
        ...
        if 'def transform(' not in v or 'return' not in v:
            raise ValueError(
                'Extractor Script должен содержать функцию transform (def transform(user_obj):) с оператором return'
            )
        if 'async' in v:
