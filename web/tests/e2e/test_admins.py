...
async def test_set_new_password(client, db_seed):
    ...
    resp = await client.put(
        "/api/v1/server/admins/passw/set_new_passw", json={"admin_id": test_admin_id, "passw": new_password}
    )
    assert resp.status_code == 200
    ...
    old_login = await client.post("/api/v1/public/admins/login", json={"login": test_login, "passw": old_password})
    assert old_login.status_code == 401, "Old password should not work"

    # Проверяем, что новый пароль работает
    new_login = await client.post("/api/v1/public/admins/login", json={"login": test_login, "passw": new_password})
