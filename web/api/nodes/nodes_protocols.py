...
from web.schemas.nodes_protocols_schema import NodeProtocolCreateSchema, UpdateNodeProtoSchema
...
async def update_node_protocol_api(body: UpdateNodeProtoSchema, db: PgSqlDep, request: Request, _: JWTCookieDep):
    ...
    На фронте при вызове этого эндпоинта фиксировать, передаются ли metrics_port или proto_port.
    ...
    log_event(f"Виртуальная нода обновлена | node_proto_id: \033[32m{body.node_proto_id}\033[0m; node_proto_body: \033[37m{repr(body)}\033[0m;admin_id: \033[32m{request.state.admin_id}\033[0m", request=request)
