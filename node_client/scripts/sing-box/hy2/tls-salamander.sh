...
if [ -z "$CERT_PATH" ] || [ -z "$KEY_PATH" ] || [ -z "$SNI_DOMAIN" ]; then
    ...
    log "Использование: bash node_client/scripts/hysteria2-native/hy2/tls-salamander.sh <cert_path> <key_path> <sni_domain>"
    exit 1
fi
...
CONFIG_PATH="$CONFIG_DIR/hy2-tls-salamander-${NODE_PROTO_ID}.json" # конфиги не должны совпасть
...
[Unit]
Description=Sing-box Hysteria2 Salamander Node (PROTO_ID: ${PROTO_ID})
