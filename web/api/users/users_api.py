...
async def get_users(
    ...
    log_event(
        f'Отдали пользователей | records: {len(users)}; sort_by: \033[32m{sort_by}\033[0m; last_id: \033[35m{last_id}\033[0m; admin_id: \033[31m{request.state.admin_id}\033[0m',
        request=request,
    )
...
async def get_user(user_id: int, request: Request, db: PgSqlDep, _: JWTCookieDep):
    ...
    if not user:
        log_event(
            f'Не удалось найти пользователя | user_id: \033[32m{user_id}\033[0m; admin_id: \033[31m{request.state.admin_id}\033[0m',
            request=request,
            level='WARNING',
        )
    ...
    log_event(
        f'Выдали Extent Юзера | user_id: \033[32m{user_id}\033[0m; admin_id: \033[31m{request.state.admin_id}\033[0m',
        request=request,
    )
...
async def bulk_create_users(body: UserBulkCreateSchema, request: Request, db: PgSqlDep, _: JWTCookieDep):
    ...
    log_event(
        f'Bulk create пользователей | users_len: {len(body.users)}; admin_id: \033[31m{request.state.admin_id}\033[0m',
        request=request,
    )
    ...
    log_event(
        f'Создано пользователей | created_users_len: {len(created_users)}; admin_id: \033[32m{request.state.admin_id}\033[0m',
        request=request,
    )
    return {'success': True, 'message': f'Пользователи созданы!', 'users': created_users}  # noqa: F541
...
async def bulk_update_users(body: UserBulkUpdateSchema, request: Request, db: PgSqlDep, arq: ArqDep, _: JWTCookieDep):
    ...
    """
    log_event(
        f'Bulk update пользователей | action: \033[35m{body.action}\033[0m; users_affected: {len(body.user_ids)}; admin_id: \033[31m{request.state.admin_id}\033[0m',
        request=request,
    )
    ...
    log_event(
        f'Обновлено пользователей ({len(affected_users)}). Закинули исполнение операции в фон | job_ids: \033[31m{job_ids}\033[0m; action: \033[32m{body.action}\033[0m; admin_id: \033[32m{request.state.admin_id}\033[0m',
        request=request,
    )
    return {
        'success': True,
        'message': f'Bulk Операция ({body.action}) выполнена',
...
async def bulk_delete_users(body: UserBulkDeleteSchema, request: Request, db: PgSqlDep, arq: ArqDep, _: JWTCookieDep):
    ...
    """
    log_event(
        f'Bulk delete пользователей | count: {len(body.user_ids)}; admin_id: \033[31m{request.state.admin_id}\033[0m',
        request=request,
    )
    ...
    log_event(
        f'Удалено пользователей: {len(delete_events)}; admin_id: \033[32m{request.state.admin_id}\033[0m',
        request=request,
        level='WARNING',
    )
    return {
        ...
        'deleted_count': len(delete_events),
        'arq_job_ids': job_ids,
    }
...
async def edit_user(user_id: int, body: UserUpdateSchema, request: Request, db: PgSqlDep, arq: ArqDep, _: JWTCookieDep):
    ...
    if status_code == 409:
        log_event(
            f'Ограничение уникальности | err: \033[31m{upd}\033[0m; admin_id: \033[31m{request.state.admin_id}\033[0m',
            request=request,
            level='WARNING',
        )
        raise HTTPException(
            status_code=409, detail={'success': False, 'message': 'Ограничение уникальности', 'err': upd}
        )

    if not upd:
        log_event(
            f'Не удалось обновить пользователя, не существует | upd_body: \033[34m{repr(body)}\033[0m; admin_id: \033[31m{request.state.admin_id}\033[0m',
            request=request,
        )
...
async def edit_user_sub(
    user_id: int, body: UserSubsUpdateSchema, request: Request, db: PgSqlDep, arq: ArqDep, _: JWTCookieDep
):
    ...
    if user_subs_del:
        ...
        log_event(
            f'\033[35m[User Subs Editor]\033[0m Отправка удаления впн-пользователей в фон | user_id: \033[34m{user_id}\033[0m; admin_id: \033[31m{request.state.admin_id}\033[0m',
            request=request,
        )
    ...
    if users_subs_add:
        log_event(
            f'\033[35m[User Subs Editor]\033[0m Отправка вставок подписок пользователя в фон на впн-ядра | user_id: \033[34m{user_id}\033[0m; admin_id: \033[31m{request.state.admin_id}\033[0m',
            request=request,
        )
