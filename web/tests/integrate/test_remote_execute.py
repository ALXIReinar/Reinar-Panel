...
class TestRemoteExecuteSuccess:
    ...
    async def test_execute_success(self, client, virtual_node_seed, db_pool, flush_redis):
        ...
        async with db_pool.acquire() as conn:
            await conn.execute("INSERT INTO whitelist_commands (command, is_active) VALUES ($1, $2)", "systemctl", True)
        ...
        async with db_pool.acquire() as conn:
            node_data = await conn.fetchrow("SELECT n.private_ip, n.api_port FROM nodes n WHERE n.id = $1", node_id_1)
        ...
        async with db_pool.acquire() as conn:
            history = await conn.fetchrow(
                "SELECT * FROM remote_execute_history WHERE command = $1", "systemctl status nginx"
            )
    ...
    async def test_execute_history_saved(self, client, virtual_node_seed, db_pool, flush_redis):
        ...
        # Добавляем команду в whitelist (только base_command - первое слово)
        async with db_pool.acquire() as conn:
            await conn.execute("INSERT INTO whitelist_commands (command, is_active) VALUES ($1, $2)", "df", True)
        ...
        async with db_pool.acquire() as conn:
            history_count = await conn.fetchval(
                "SELECT COUNT(*) FROM remote_execute_history WHERE command = $1", "df -h"
            )
...
class TestRemoteExecuteErrors:
    ...
    async def test_execute_not_whitelisted(self, client, virtual_node_seed, db_pool, flush_redis):
        ...
        async with db_pool.acquire() as conn:
            await conn.execute("INSERT INTO whitelist_commands (command, is_active) VALUES ($1, $2)", "ls", True)
        ...
        async with db_pool.acquire() as conn:
            history_count = await conn.fetchval("SELECT COUNT(*) FROM remote_execute_history")
    ...
    async def test_execute_node_unreachable(self, client, virtual_node_seed, db_pool, flush_redis):
        ...
        # Добавляем команду в whitelist (только base_command - первое слово)
        async with db_pool.acquire() as conn:
            await conn.execute("INSERT INTO whitelist_commands (command, is_active) VALUES ($1, $2)", "ps", True)
        ...
        async with db_pool.acquire() as conn:
            history = await conn.fetchrow("SELECT * FROM remote_execute_history WHERE command = $1", "ps aux")
            assert history is not None
    ...
    async def test_execute_node_error_response(self, client, virtual_node_seed, db_pool, flush_redis):
        ...
        client.app.state.cmd_center_aiohttp = FakeAiohttpSession(json_data={"error": "Not Found"}, status=404)
        ...
        async with db_pool.acquire() as conn:
            history = await conn.fetchrow("SELECT * FROM remote_execute_history WHERE command = $1", "ls /nonexistent")
            assert history is not None
...
class TestRemoteExecuteHistoryUpdates:
    ...
    async def test_execute_history_updated_on_success(self, client, virtual_node_seed, db_pool, flush_redis):
        ...
        async with db_pool.acquire() as conn:
            history = await conn.fetchrow("SELECT * FROM remote_execute_history WHERE command = $1", "echo test")
            assert history is not None
    ...
    async def test_execute_history_updated_on_error(self, client, virtual_node_seed, db_pool, flush_redis):
        ...
        # Добавляем команду в whitelist (только base_command - первое слово)
        async with db_pool.acquire() as conn:
            await conn.execute("INSERT INTO whitelist_commands (command, is_active) VALUES ($1, $2)", "cat", True)
        ...
        async with db_pool.acquire() as conn:
            history = await conn.fetchrow("SELECT * FROM remote_execute_history WHERE command = $1", "cat /etc/shadow")
            assert history is not None
