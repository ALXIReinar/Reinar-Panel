...
from web.sub.config_dir.config import env  # noqa: F401
...
class TestAddTgUser:
    ...
    async def test_add_new_tg_user_with_return_data(self, test_tg_routing_client, tg_routing_seed):
        ...
        """
        # Arrange
        payload = {'tg_id': 999001, 'tg_username': 'new_test_user', 'return_data': True}
        # Act
    ...
    async def test_add_new_tg_user_without_return_data(self, test_tg_routing_client, tg_routing_seed):
        ...
        """
        # Arrange
        payload = {'tg_id': 999002, 'tg_username': 'another_new_user', 'return_data': False}
        # Act
        response = await test_tg_routing_client.post('/tg/users/add', json=payload)
        # Assert
    ...
    async def test_add_existing_user_updates_username(self, test_tg_routing_client, tg_routing_seed, db_pool):
        ...
        old_username = tg_routing_seed['user_with_subs']['tg_username']  # noqa: F841
...
class TestGetTgUser:
    ...
    async def test_get_tg_user_profile_success(self, test_tg_routing_client, tg_routing_seed):
        ...
        """
        # Act
        response = await test_tg_routing_client.get(f'/tg/users/get?tg_id={tg_routing_seed["user_with_subs"]["tg_id"]}')
        # Assert
        assert response.status_code == 200
        data = response.json()
        assert 'id' in data
    ...
    async def test_get_tg_user_profile_no_subs(self, test_tg_routing_client, tg_routing_seed):
        ...
        """
        # Act
        response = await test_tg_routing_client.get(f'/tg/users/get?tg_id={tg_routing_seed["user_no_subs"]["tg_id"]}')
        # Assert
        assert response.status_code == 200
        data = response.json()
        assert 'id' in data
...
class TestTgRoutingAuthorization:
            ...
            try:
                ...
                async with httpx.AsyncClient(transport=transport, base_url="http://test") as ac:
                    ...
                    response1 = await ac.post(
                        '/tg/users/add', json={'tg_id': 123, 'tg_username': 'test', 'return_data': False}
                    )
    ...
    async def test_endpoints_accept_authorized_ip(self, test_tg_routing_client, tg_routing_seed):
        ...
        # Test POST /users/add
        response1 = await test_tg_routing_client.post(
            '/tg/users/add', json={'tg_id': 999888, 'tg_username': 'authorized_test', 'return_data': False}
        )
