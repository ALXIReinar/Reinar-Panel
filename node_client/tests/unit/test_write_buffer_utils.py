    ...
    return {
        ...
        "inbounds": [
            {"port": 10085, "protocol": "dokodemo-door", "tag": "api"},
            {
                ...
                "settings": {
                    "clients": [
                        {"id": "uuid-001", "email": "user1@test.com", "flow": "xtls-rprx-vision"},
                        {"id": "uuid-002", "email": "user2@test.com", "flow": "xtls-rprx-vision"},
                    ],
                    "decryption": "none",
                },
            },
...
def test_flatten_key2value_with_replace(sample_xray_config):
    ...
    result = flatten_key2value(
        sample_xray_config, "inbounds___1___settings___clients", new_last_obj=new_clients, replace_last_obj=True
    )
...
def test_navigate_to_path_not_array(sample_xray_config):
    ...
    with pytest.raises(TypeError, match="не указывает на массив"):
        ...
        buffer._navigate_to_path(sample_xray_config, "inbounds___1___settings")
...
async def test_write_config_atomic_uses_tmp_dir(tmp_path, sample_xray_config):
    ...
    tmp_files = list(TMP_DIR.glob("*.tmp"))  # noqa: F841
