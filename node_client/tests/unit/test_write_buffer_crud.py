...
def empty_config(tmp_path):
    ...
    config = {"inbounds": [{"port": 443, "protocol": "vless", "settings": {"clients": []}}]}
...
async def test_add_user_registration_fails_invalid_identifier(config_with_3_users):
    ...
    bad_injectors = create_user_injectors(extractor_script="def wrong_function_name(u): return u")
    success, status_code, msg = await buffer.add_user(
        node_proto_id=node_proto_id,
        user_obj=user,
        filepath=str(config_with_3_users),
...
async def test_bulk_action_add_small_batch(config_with_3_users):
    ...
    new_users = [
        create_test_user(email=f"bulk{i}@test.com", uuid=f"uuid-bulk-{i}", as_superuser=True) for i in range(5)
    ]
    # Выполняем bulk add
...
async def test_bulk_action_add_large_batch(config_with_3_users):
    ...
    new_users = [
        create_test_user(email=f"bulk{i}@test.com", uuid=f"uuid-bulk-{i}", as_superuser=True) for i in range(20)
    ]
    # Выполняем bulk add
    success, msg = await buffer.bulk_action(
        node_proto_id=node_proto_id,
    ...
    queue = buffer.node_queues[node_proto_id]  # noqa: F841
...
async def test_bulk_action_unregistered_node(empty_config):
    ...
    new_users = [create_test_user(email=f"user{i}@test.com", uuid=f"uuid-{i}", as_superuser=True) for i in range(3)]
    success, msg = await buffer.bulk_action(
        node_proto_id=node_proto_id,
        users=new_users,
        filepath=str(empty_config),
...
async def test_bulk_action_registration_fails(tmp_path):
    ...
    new_users = [create_test_user(email="user@test.com", uuid="uuid-1", as_superuser=True)]
    success, msg = await buffer.bulk_action(
        node_proto_id=node_proto_id,
        users=new_users,
        filepath=str(broken_config),
