...
class CommandWhitelistCache:
    ...
    async def get_base_whitelist(cls, redis: Redis, cmd_only: bool = False) -> list[str]:
        ...
        try:
            ...
            if cached is not None:
                ...
                if cmd_only:
                    return [
                        id_cmd_active['command'] for id_cmd_active in ids_cmds if id_cmd_active['is_active'] == True
                    ]
                return ids_cmds
            return []
    ...
    async def set_base_whitelist(cls, redis: Redis, db: PgSqlDep) -> bool:
        try:
            ...
            base_list = [
                {'id': row['id'], 'command': row['command'], 'is_active': row['is_active']} for row in base_commands
            ]
    ...
    async def is_whitelisted(cls, command: str, redis: Redis, db) -> bool:
        ...
        if cls.whitelist_mode:
            ...
            log_event(
                f'Проверяем команду на наличие в белом списке | base_cmd: \033[32m{base_cmd}\033[0m; wh_status: \033[34m{cls.whitelist_mode}\033[0m'
            )
