...
from web.schemas.proto_commands_schema import (
...
@router.get('/{proto_id}', summary="Получить команды протокола")
async def get_proto_commands(proto_id: int, db: PgSqlDep, request: Request, _: JWTCookieDep):
    ...
    log_event(
        f'Отдали команды для протокола | cmds_len: \033[34m{len(commands)}\033[0m; proto_id: \033[32m{proto_id}\033[0m; admin_id: \033[31m{request.state.admin_id}\033[0m',
        request=request,
    )
    return {'commands': commands}


@router.post('/bulk/add', summary="Массовая вставка команд")
async def bulk_insert_commands(body: CommandsBulkInsertSchema, db: PgSqlDep, request: Request, _: JWTCookieDep):
    ...
    if status_code == 404:
        log_event(
            f"Bulk insert отклонён. Протокол не найден! | proto_id: \033[31m{body.proto_id}\033[0m; admin_id: \033[31m{request.state.admin_id}\033[0m",
            request=request,
            level='WARNING',
        )
    ...
    log_event(
        f"Bulk insert команд | proto_id: \033[33m{body.proto_id}\033[0m; inserted: \033[32m{cmd_ids}\033[0m; admin_id: \033[31m{request.state.admin_id}\033[0m",
        request=request,
    )
...
async def bulk_update_commands(body: CommandsBulkUpdateSchema, db: PgSqlDep, request: Request, _: JWTCookieDep):
    ...
    log_event(
        f"Bulk update команд | updated: \033[32m{updated_cmds}\033[0m; admin_id: \033[31m{request.state.admin_id}\033[0m",
        request=request,
    )
...
async def bulk_delete_commands(body: CommandsBulkDeleteSchema, db: PgSqlDep, request: Request, _: JWTCookieDep):
    ...
    log_event(
        f"Bulk delete команд | deleted: \033[31m{deleted_count}\033[0m; cmd_ids: \033[33m{body.cmd_ids}\033[0m; admin_id: \033[31m{request.state.admin_id}\033[0m",
        request=request,
        level='WARNING',
    )
