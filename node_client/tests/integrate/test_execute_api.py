...
async def test_execute_simple_command(client):
    ...
    response = await client.post("/api/v1/server/node/execute", json={"command": "echo Hello World"})
...
async def test_execute_command_with_exit_code_0(client):
    ...
    if platform.system() == "Windows":
        ...
        command = "pwd"
    response = await client.post("/api/v1/server/node/execute", json={"command": command})
    assert response.status_code == 200
    data = response.json()
    assert data["success"] is True
    assert data["exit_code"] == 0
...
async def test_execute_command_with_stdout(client):
    ...
    response = await client.post("/api/v1/server/node/execute", json={"command": "echo Test Output Line"})
    assert response.status_code == 200
    data = response.json()
    assert data["success"] is True
...
async def test_execute_command_with_stderr(client):
    ...
    response = await client.post(
        "/api/v1/server/node/execute", json={"command": "powershell -Command \"Write-Error 'Test Error Message'\""}
    )
    assert response.status_code == 200
    data = response.json()
...
async def test_execute_command_non_zero_exit(client):
    ...
    response = await client.post("/api/v1/server/node/execute", json={"command": "exit 1"})
    assert response.status_code == 200
    data = response.json()
...
async def test_execute_command_timeout(client):
    ...
    with patch('node_client.api.execute_api.subprocess.run') as mock_run:
        ...
        response = await client.post("/api/v1/server/node/execute", json={"command": "sleep 100"})
...
async def test_execute_invalid_command(client):
    ...
    response = await client.post("/api/v1/server/node/execute", json={"command": "thisisnotarealcommand12345xyz"})
    assert response.status_code == 200
    data = response.json()
...
async def test_execute_powershell_command(client):
    ...
    response = await client.post(
        "/api/v1/server/node/execute", json={"command": "powershell -Command \"Write-Output 'PowerShell Test'\""}
    )
    assert response.status_code == 200
    data = response.json()
    assert data["success"] is True
...
async def test_execute_command_with_args(client):
    ...
    response = await client.post("/api/v1/server/node/execute", json={"command": "echo First Second Third Fourth"})
    assert response.status_code == 200
    data = response.json()
    assert data["success"] is True
...
async def test_execute_multiline_output(client):
    ...
    response = await client.post(
        "/api/v1/server/node/execute", json={"command": "echo Line1 & echo Line2 & echo Line3"}
    )
    assert response.status_code == 200
    data = response.json()
    assert data["success"] is True
...
async def test_execute_command_preserves_command_string(client):
    ...
    response = await client.post("/api/v1/server/node/execute", json={"command": test_command})
    assert response.status_code == 200
    data = response.json()
...
async def test_execute_command_with_special_characters(client):
    ...
    response = await client.post("/api/v1/server/node/execute", json={"command": "echo Hello@World#Test$"})
    assert response.status_code == 200
    data = response.json()
    assert data["success"] is True
...
async def test_execute_command_exception_handling(client):
    ...
    with patch('node_client.api.execute_api.subprocess.run') as mock_run:
        ...
        response = await client.post("/api/v1/server/node/execute", json={"command": "any command"})
...
async def test_get_metrics_cli_command_timeout(client, mock_buffer):
    ...
    with patch('node_client.api.execute_api.subprocess.run') as mock_subprocess:
        # CLI команда timeout
        mock_subprocess.side_effect = subprocess.TimeoutExpired(cmd="xray api statsquery", timeout=10)
        response = await client.post(
            "/api/v1/server/node/metrics",
            json={
                "node_proto_id": node_proto_id,
