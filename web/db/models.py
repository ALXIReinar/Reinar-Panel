...
class ProtoTemplates(Base):
    ...
    api_add_user_script: Mapped[Optional[str]] = mapped_column(Text)
    api_delete_user_script: Mapped[Optional[str]] = mapped_column(Text)
    proto_python_lib: Mapped[Optional[str]] = mapped_column(String(32))
    flatten_json_users_key: Mapped[Optional[str]] = mapped_column(String(1024))
    flatten_user_identifier_key: Mapped[Optional[str]] = mapped_column(String(128))
    ...
    add_script_custom_params: Mapped[Optional[dict]] = mapped_column(JSONB, server_default=text("'{}'::jsonb"))
    delete_script_custom_params: Mapped[Optional[dict]] = mapped_column(JSONB, server_default=text("'{}'::jsonb"))
