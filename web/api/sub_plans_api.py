...
async def create_sub_plan(body: SubPlanCreateSchema, request: Request, db: PgSqlDep, _: JWTCookieDep):
    ...
    if not plan_id:
        log_event(
            f'Подписка с таким названием уже существует | plan_name: \033[32m{body.title}\033[0m; admin_id: \033[31m{request.state.admin_id}\033[0m',
            request=request,
            level='WARNING',
        )
        raise HTTPException(
            status_code=409, detail={'success': False, 'message': 'Подписка с таким именем уже существует'}
        )

    log_event(
        f'Группа подписок создана | plan_id: \033[32m{plan_id}\033[0m; admin_id: \033[31m{request.state.admin_id}\033[0m',
        request=request,
    )
...
async def update_sub_plan(plan_id: int, body: SubPlanUpdateSchema, request: Request, db: PgSqlDep, _: JWTCookieDep):
    ...
    """
    log_event(
        f'Обновление группы подписок | plan_id: \033[35m{plan_id}\033[0m; body: \033[37m{repr(body)}\033[0m; admin_id: \033[31m{request.state.admin_id}\033[0m',
        request=request,
    )
    ...
    if not plan_updated:
        log_event(
            f'Группа подписок не найдена | plan_id: \033[33m{plan_id}\033[0m; admin_id: \033[31m{request.state.admin_id}\033[0m',
            request=request,
            level='WARNING',
        )
    ...
    log_event(
        f'Группа подписок обновлена | plan_id: \033[32m{plan_id}\033[0m; admin_id: \033[32m{request.state.admin_id}\033[0m',
        request=request,
    )
    return {
        'success': True,
...
async def delete_sub_plan(plan_id: int, request: Request, db: PgSqlDep, _: JWTCookieDep):
    ...
    log_event(
        f'Удаление группы подписок | plan_id: \033[35m{plan_id}\033[0m; admin_id: \033[31m{request.state.admin_id}\033[0m',
        request=request,
    )
    ...
    log_event(
        f'Группа подписок удалена | plan_id: \033[32m{plan_id}\033[0m; admin_id: \033[32m{request.state.admin_id}\033[0m',
        request=request,
        level='WARNING',
    )
...
async def get_all_sub_plans(request: Request, db: PgSqlDep, _: JWTCookieDep, limit: int = 20, offset: int = 0):
    ...
    log_event(
        f'Отдали список групп подписок | plans_len: \033[32m{len(plans)}\033[0m; admin_id: \033[31m{request.state.admin_id}\033[0m',
        request=request,
    )
...
async def get_sub_plan(plan_id: int, request: Request, db: PgSqlDep, _: JWTCookieDep):
    ...
    if not plan:
        log_event(
            f"Не нашли группу подписок | plan_id: \033[31m{plan_id}\033[0m; admin_id: \033[31m{request.state.admin_id}\033[0m",
            request=request,
            level='WARNING',
        )
    ...
    log_event(
        f'Отдали группу подписок | plan_id: \033[32m{plan_id}\033[0m; admin_id: \033[31m{request.state.admin_id}\033[0m',
        request=request,
    )
...
async def edit_vnodes_loadout(
    plan_id: int, body: SubPlanVnodesSetSchema, request: Request, db: PgSqlDep, _: JWTCookieDep, arq: ArqDep
):
    """
    ...
    if attached:
        ...
        log_event(
            f"Закинули в фон вставку впн-пользователей на новые ноды(локации) | sub_plan_id: \033[35m{plan_id}\033[0m; job_id: \033[31m{job.job_id}\033[0m; admin_id: \033[31m{request.state.admin_id}\033[0m",
            request=request,
        )
        add_job = job.job_id
    ...
    if detached:
        ...
        log_event(
            f"Закинули в фон удаление впн-пользователей с только что удалённых локаций | sub_plan_id: \033[36m{plan_id}\033[0m; job_id: \033[31m{job.job_id}\033[0m; admin_id: \033[31m{request.state.admin_id}\033[0m",
            request=request,
        )
        del_job = job.job_id
