...
CONFIG_PATH="$CONFIG_DIR/ss-tls-tcp-${NODE_PROTO_ID}.json"
cat <<EOF > "$CONFIG_PATH"
{
  "log": {
      "level": "warn"
...
[Unit]
Description=Sing-box Shadowsocks TLS TCP EXIT Node (NODE_PROTO_ID: ${NODE_PROTO_ID})
...
log "=================================================="
log "Sing-box Shadowsocks-tcp TLS TCP node в качестве EXIT ноды развернута!"
