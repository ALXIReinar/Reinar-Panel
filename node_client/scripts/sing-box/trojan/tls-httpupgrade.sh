...
CONFIG_DIR="/etc/reinar/configs/sing-box/trojan"
...
HTTPUP_PATH="/trojan-$(openssl rand -hex 4)-httpup"
...
CONFIG_PATH="$CONFIG_DIR/trojan-tls-httpup-${NODE_PROTO_ID}.json"
cat <<EOF > "$CONFIG_PATH"
{
  "log": {
      "level": "warn"
...
[Unit]
Description=Sing-box Trojan HttpUpgrade TLS Node (NODE_PROTO_ID: ${NODE_PROTO_ID})
