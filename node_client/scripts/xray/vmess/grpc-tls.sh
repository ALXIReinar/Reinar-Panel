...
if [ -z "$TMP_ID" ] || [ -z "$DOMAIN" ] || [ -z "$CERT_PATH" ] || [ -z "$KEY_PATH" ]; then
    ...
    echo "Использование: bash vmess-grpc-tls.sh <tmp_id> <domain> <cert_path> <key_path>"
    exit 1
fi
...
SERVICE_NAME="vmess-grpc-${TMP_ID}"
...
[Unit]
Description=Xray VMess-gRPC-TLS Node (TMP_ID: ${TMP_ID})
...
curl -s -X POST "$PANEL_CALLBACK_URL" \
     ...
     -d '{
           ...
           "status": "installed",
           "node_type": "vmess_grpc_tls",
           "custom_fields": {
               "grpc_type": "gun"
           }
         }'

echo "VMess-gRPC-TLS нода $TMP_ID успешно установлена."