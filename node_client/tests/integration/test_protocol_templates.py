...
# ========== Группа 4: Subscription URL генерация (sub_prepare_script) ==========

@pytest.mark.asyncio
@pytest.mark.db
async def test_template_sub_prepare_script_execution(protocol_templates_with_extractors):
    """
    Проверка: sub_prepare_script генерирует валидную конфигурационную ссылку для VPN клиента
    
    Тестируемый функционал:
    - sub_prepare_script существует для каждого is_accepted шаблона (не NULL)
    - Скрипт компилируется и определяет функцию prepare_sub()
    - Функция выполняется без ошибок
    - UUID пользователя корректно подставляется в ссылку
    - Результат имеет правильный формат протокола (vless://, vmess://, ss://, trojan://, hy2://)
    
    Формат входных данных:
    - user_uuid: str (настоящий UUID4)
    - url_tmp: str (шаблон ссылки из БД с плейсхолдерами {{node___address}}, {user_uuid}, и др.)
    
    Формат выходных данных (зависит от протокола):
    - VLESS/Trojan: простая подстановка → vless://{uuid}@host:port?params#name
    - VMess: JSON → Base64 кодирование → vmess://base64_here
    - Shadowsocks: JSON → генерация PSK из UUID → ss://base64@host:port#name
    - Hysteria2: простая подстановка → hysteria2://{uuid}@host:port?params#name
    
    ВАЖНО: 
    - url_tmp берётся из БД "как есть" (строка с плейсхолдерами)
    - Скрипт сам парсит JSON если нужно (например, для shadowsocks)
    - HotReloadExecutor автоматически импортирует библиотеки из sub_required_libs
    
    Запуск:
        pytest node_client/tests/integration/test_protocol_templates.py::test_template_sub_prepare_script_execution --protocol=* -v
        pytest node_client/tests/integration/test_protocol_templates.py::test_template_sub_prepare_script_execution --protocol=xray -v
    """
    import uuid as uuid_lib
    import base64
    import json
    ...
    for template in protocol_templates_with_extractors:
        ...
        sub_script = template.get('sub_prepare_script')
        assert sub_script, (
            f"Template '{template['title']}': "
            f"sub_prepare_script не может быть NULL для is_accepted=true шаблонов"
        )
        
        url_tmp = template.get('url_tmp')
        assert url_tmp, (
            f"Template '{template['title']}': "
            f"url_tmp не может быть NULL"
        )
        
        # 2. Компилируем скрипт через HotReloadExecutor
        lib_names = template.get('sub_required_libs')
        
        try:
            compiled_func = HotReloadExecutor.get_compiled_func(
                func_script=sub_script,
                func_name='prepare_sub',
                libs=lib_names
            )
        except Exception as e:
            pytest.fail(
                f"Template '{template['title']}': "
                f"Не удалось скомпилировать sub_prepare_script: {e}\n"
                f"Libs: {lib_names}"
            )
        
        # 3. Подготавливаем тестовые данные
        # Генерируем настоящий UUID4 (критично для shadowsocks и других скриптов)
        ...
        try:
            ...
            pytest.fail(
                f"Template '{template['title']}': "
                f"Ошибка выполнения prepare_sub(): {e}\n"
                f"UUID: {test_uuid}\n"
                f"URL template (first 200 chars): {url_tmp[:200]}"
            )
        
        # 5. Базовые проверки результата
        assert isinstance(result, str), (
            f"Template '{template['title']}': "
            f"prepare_sub() должен возвращать строку, получено: {type(result)}"
        )
        
        assert len(result) > 0, (
            f"Template '{template['title']}': "
            f"prepare_sub() вернул пустую строку"
        )
        
        # 6. Проверяем что UUID подставлен (нет плейсхолдера)
        assert '{user_uuid}' not in result, (
            f"Template '{template['title']}': "
            f"UUID не был подставлен в результат.\n"
            f"Result (first 150 chars): {result[:150]}"
        )
        
        # 7. Проверяем формат ссылки в зависимости от протокола
        title_lower = template['title'].lower()
        
        if 'vless' in title_lower:
            assert result.startswith('vless://'), (
                f"Template '{template['title']}': "
                f"Ожидалась ссылка vless://, получено: {result[:50]}"
            )
            # Проверяем что UUID присутствует в ссылке
            assert test_uuid in result, (
                f"Template '{template['title']}': "
                f"UUID не найден в результирующей ссылке"
            )
        
        elif 'vmess' in title_lower:
            assert result.startswith('vmess://'), (
                f"Template '{template['title']}': "
                f"Ожидалась ссылка vmess://, получено: {result[:50]}"
            )
            # Дополнительно проверяем что Base64 декодируется корректно
            try:
                base64_part = result.replace('vmess://', '')
                decoded = base64.b64decode(base64_part)
                decoded_json = json.loads(decoded)
                
                # Проверяем что JSON содержит UUID в поле id
                uuid_field = decoded_json.get('id') or decoded_json.get('uuid')
                assert uuid_field, "Декодированный JSON должен содержать поле 'id' или 'uuid'"
                assert test_uuid in uuid_field or uuid_field in test_uuid, (
                    f"UUID не найден в декодированном JSON: {decoded_json}"
                )
            except Exception as e:
                pytest.fail(
                    f"Template '{template['title']}': "
                    f"Невалидный Base64 или JSON в vmess:// ссылке: {e}\n"
                    f"Result: {result[:100]}"
                )
            ...
            assert result.startswith('trojan://'), (
                f"Template '{template['title']}': "
                f"Ожидалась ссылка trojan://, получено: {result[:50]}"
            )
            # Проверяем что UUID присутствует (после trojan:// до @)
            assert test_uuid in result, (
                f"Template '{template['title']}': "
                f"UUID не найден в результирующей ссылке"
            )
        
        elif 'shadowsocks' in title_lower:
            assert result.startswith('ss://'), (
                f"Template '{template['title']}': "
                f"Ожидалась ссылка ss://, получено: {result[:50]}"
            )
            # Для shadowsocks UUID преобразуется в PSK через хеширование,
            # поэтому прямого вхождения test_uuid в result не будет
            # Проверяем только что ссылка имеет правильный формат: ss://base64@host:port#title
            parts = result.split('@')
            assert len(parts) == 2, (
                f"Template '{template['title']}': "
                f"ss:// ссылка должна содержать '@' разделитель"
            )
            # Проверяем что первая часть (после ss://) это Base64
            try:
                base64_part = parts[0].replace('ss://', '')
                base64.b64decode(base64_part)
            except Exception as e:
                pytest.fail(
                    f"Template '{template['title']}': "
                    f"Невалидный Base64 в ss:// ссылке: {e}"
                )
        
        elif 'hy2' in title_lower or 'hysteria2' in title_lower:
            assert result.startswith('hysteria2://') or result.startswith('hy2://'), (
                f"Template '{template['title']}': "
                f"Ожидалась ссылка hysteria2:// или hy2://, получено: {result[:50]}"
            )
            # Проверяем что UUID присутствует
            assert test_uuid in result, (
                f"Template '{template['title']}': "
                f"UUID не найден в результирующей ссылке"
            )
            ...
            # Для неизвестных протоколов проверяем только базовые вещи
            # (ссылка не пустая и UUID как-то использован)
            pass
