...
class TestCreatePhysicalNode:
    ...
    async def test_create_node_success(self, client, db_seed):
        ...
        client.app.state.cmd_center_aiohttp = FakeAiohttpSession(
            json_data={"success": True, "message": "pong", "service": "test-node-service", "version": "0.1"}
        )
    ...
    async def test_create_node_invalid_response(self, client, db_seed):
        ...
        client.app.state.cmd_center_aiohttp = FakeAiohttpSession(json_data={"message": "pong"})
    ...
    async def test_create_node_duplicate(self, client, db_seed):
        ...
        client.app.state.cmd_center_aiohttp = FakeAiohttpSession(
            json_data={"success": True, "message": "pong", "service": "duplicate-node", "version": "0.1"}
        )
...
class TestUpdatePhysicalNode:
    ...
    async def test_update_node_partial(self, client, physical_node_seed):
        ...
        response = await client.put(
            f"/api/v1/private/nodes/update/{node_id}",
            json={"node_id": node_id, "title": "Partially Updated", "is_active": True},
        )
    ...
    async def test_update_node_conflict(self, client, physical_node_seed):
        ...
        response = await client.put(
            ...
            json={"node_id": node_id_1, "ip": "192.168.1.101", "private_ip": "10.0.0.101", "api_port": 8101},
        )
    ...
    async def test_delete_node_cascade(self, client, physical_node_seed, db_pool, proto_template_seed):
        ...
        async with db_pool.acquire() as conn:
            vnode_exists_before = await conn.fetchval(
                "SELECT EXISTS(SELECT 1 FROM nodes_protocols WHERE id = $1)", vnode_id
            )
