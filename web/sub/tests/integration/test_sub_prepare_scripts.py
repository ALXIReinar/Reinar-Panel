...
async def test_sub_prepare_script_execution(sub_prepare_infrastructure):
    ...
    for template_id, data in sub_prepare_infrastructure.items():
        ...
        try:
            ...
            try:
                conf_url = render_config_link_for_test(url_tmp=template['url_tmp'], node_config_json=mock_config)
            ...
            if not ok:
                errors.append({'template': template['title'], 'step': 'create_user_super_obj', 'error': user_super_obj})
                continue
    ...
    print(f"\n{'=' * 80}")
    print(f"Sub Prepare Scripts Test Report")  # noqa: F541
    print(f"{'=' * 80}")
...
async def test_sub_prepare_infrastructure_fixture(sub_prepare_infrastructure):
    ...
    for template_id, data in sub_prepare_infrastructure.items():
        ...
        print(
            f"✓ Template {template['title']} (id={template_id}): "
...
async def test_sub_prepare_no_double_encoding(sub_prepare_infrastructure):
    ...
    for template_id, data in sub_prepare_infrastructure.items():
        ...
        try:
            ...
            success, final_link = await ScriptExecutor.executing_link_processing(
                ...
                required_libs=template['sub_required_libs'],
                user_obj=user_super_obj,
                config_link={'conf_url': conf_url, 'n_address': node_address, 'n_title': node_title},
            )
            if not success:
    ...
    print(f"\n{'=' * 80}")
    print(f"Double Encoding Check Report")  # noqa: F541
    print(f"{'=' * 80}")
