...
class TestUpdateUserMeta:
    ...
    async def test_update_user_meta_success(self, client, user_with_subs, mock_arq, db_pool):
        ...
        response = await client.put(
            ...
            json={"tg_username": "updated_username", "tg_id": 987654321, "registered_at": "2026-01-01T00:00:00Z"},
        )
        ...
        async with db_pool.acquire() as conn:
            user = await conn.fetchrow("SELECT tg_username, tg_id, registered_at FROM users WHERE id = $1", user_id)
    ...
    async def test_update_user_meta_partial(self, client, user_with_subs, mock_arq, db_pool):
        ...
        response = await client.put(f"/api/v1/private/users/meta/{user_id}", json={"tg_username": "partial_update"})
        ...
        async with db_pool.acquire() as conn:
            user = await conn.fetchrow("SELECT tg_username, tg_id FROM users WHERE id = $1", user_id)
    ...
    async def test_update_user_meta_duplicate_tg_username(self, client, user_with_subs, mock_arq, db_pool):
        ...
        async with db_pool.acquire() as conn:
            await conn.execute("INSERT INTO users (tg_id, tg_username) VALUES ($1, $2)", 999999999, "existing_user")

        # Пытаемся обновить на существующий username
        response = await client.put(f"/api/v1/private/users/meta/{user_id}", json={"tg_username": "existing_user"})
    ...
    async def test_update_user_meta_not_found(self, client, db_seed, mock_arq):
        ...
        response = await client.put("/api/v1/private/users/meta/99999", json={"tg_username": "nonexistent_user"})
...
class TestUpdateUserSubs:
    ...
    async def test_update_user_subs_add_subscription(self, client, user_with_subs, mock_arq, db_pool):
        ...
        async with db_pool.acquire() as conn:
            subs_count = await conn.fetchval("SELECT COUNT(*) FROM user_subs WHERE user_id = $1", user_id)
    ...
    async def test_update_user_subs_delete_subscription(self, client, user_with_subs, mock_arq, db_pool):
        ...
        response = await client.put(
            f"/api/v1/private/users/subs/{user_id}",
            json={"user_subs_to_add": [], "user_subs_to_update": [], "user_subs_to_delete": [user_sub_id]},
        )
        ...
        async with db_pool.acquire() as conn:
            sub_exists = await conn.fetchval("SELECT COUNT(*) FROM user_subs WHERE id = $1", user_sub_id)
            assert sub_exists == 0
    ...
    async def test_update_user_subs_update_subscription(self, client, user_with_subs, db_pool):
        ...
        async with db_pool.acquire() as conn:
            sub = await conn.fetchrow("SELECT infinite_traffic FROM user_subs WHERE id = $1", user_sub_id)
    ...
    async def test_update_user_subs_combined_operations(self, client, user_with_subs, mock_arq, db_pool):
        ...
        async with db_pool.acquire() as conn:
            active_subs = await conn.fetchval(
                "SELECT COUNT(*) FROM user_subs WHERE user_id = $1 AND is_active = true", user_id
            )
