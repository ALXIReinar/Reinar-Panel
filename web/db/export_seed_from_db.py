...
async def export_seed_data():
    ...
    try:
        async with pool.acquire() as conn:
            ...
            templates_statuses = await conn.fetch("SELECT * FROM templates_statuses ORDER BY id")
            # 1. templates_statuses
            vnodes_reg_statuses = await conn.fetch("SELECT * FROM vnodes_reg_statuses ORDER BY id")
            ...
            # 6. pay_statuses
            pay_statuses = await conn.fetch("SELECT * FROM pay_statuses ORDER BY id")
            # 7. online_statuses
            online_statuses = await conn.fetch("SELECT * FROM online_statuses ORDER BY id")
            # 8. sub_nodes_operations
            sub_nodes_operations = await conn.fetch("SELECT * FROM sub_nodes_operations ORDER BY id")

            # 9. whitelist_commands
            whitelist_commands = await conn.fetch("SELECT * FROM whitelist_commands ORDER BY id")
...
async def main():
    ...
    print(f"\n📝 Для применения запустите: python web/db/seed_data.py")  # noqa: F541
