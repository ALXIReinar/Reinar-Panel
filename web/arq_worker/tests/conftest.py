...
import asyncpg  # noqa: E402, I001
import pytest  # noqa: E402
from arq import create_pool as create_arq_pool  # noqa: E402
from aiohttp import ClientResponseError, ClientError, RequestInfo  # noqa: E402
from unittest.mock import AsyncMock, MagicMock  # noqa: E402
from redis.asyncio import Redis  # noqa: E402

from web.arq_worker import config as cfg  # noqa: E402
from web.arq_worker.utils.anything import VnodeRegStatuses  # noqa: E402
    ...
    def raise_for_status(self):
        ...
        if self.status >= 400:
            ...
            raise ClientResponseError(
                request_info=request_info, history=(), status=self.status, message=f"HTTP {self.status}", headers={}
            )
...
async def proto_template_seed(db_pool):
    ...
    async with db_pool.acquire() as conn:
        ...
        if not tmp_id:
            raise RuntimeError(
                "Не найдено ни одного активного шаблона в proto_templates! Запустите: python -m web.db.seed_data"
            )
...
async def user_seed(db_pool):
    ...
    async with db_pool.acquire() as conn:
        ...
        return {"user_id": user_id, "tg_username": "test_user"}
...
async def virtual_node_seed(db_pool, physical_node_seed, proto_template_seed):
    ...
    async with db_pool.acquire() as conn:
        # Используем tmp_id напрямую, таблица protocols удалена
...
async def arq_test_seed(db_pool, db_seed):
    ...
    async with db_pool.acquire() as conn:
        ...
        # 5. Создаём виртуальные ноды (используем tmp_id напрямую, таблица protocols удалена)
        ...
        """,
            ...
            9095,
            "/etc/config-failed.json",
            False,
            VnodeRegStatuses.failed,
        )
        ...
        for i in range(3):
            ...
            """,
                ...
                pay_order,
                (i + 1) * 10,
                f"uuid-inactive-user-sub{i + 1}",
                f"b64-inactive-sub{i + 1}",
            )
        ...
        return {
            ...
            "should_unlock": {
                "user_a": {
                    "user_id": user_a_id,
                    "order_id": order_a,
                    "username": "user_a_limited",
                    "uuid": "uuid-reset-user-a",
                    "traffic_mb": 500,
                },
                "user_b": {
                    "user_id": user_b_id,
                    "order_id": order_b,
                    "username": "user_b_limited",
                    "uuid": "uuid-reset-user-b",
                    "traffic_mb": 800,
                },
            },
            ...
            "should_not_unlock": {
                "user_c": {
                    "user_id": user_c_id,
                    "order_id": order_c,
                    "username": "user_c_not_limited",
                    "reason": "not limited",
                },
                "user_d": {
                    "user_id": user_d_id,
                    "order_id": order_d,
                    "username": "user_d_inactive_sub",
                    "reason": "inactive subscription",
                },
                "user_e": {
                    "user_id": user_e_id,
                    "order_id": order_e,
                    "username": "user_e_deleted",
                    "reason": "user deleted",
                },
                "user_f": {
                    "user_id": user_f_id,
                    "order_id": order_f,
                    "username": "user_f_invisible_node",
                    "reason": "on invisible node",
                },
            },
        }
        ...
        return {
            ...
            "should_retry": {
                "outbox_a": {
                    ...
                    "user_id": user_a_id,
                    "order_id": order_a,
                    "username": "user_a_stuck",
                    "uuid": "uuid-stuck-user-a",
                },
                "outbox_b": {
                    "id": outbox_b,
                    "user_id": user_b_id,
                    "order_id": order_b,
                    "username": "user_b_stuck",
                    "uuid": "uuid-stuck-user-b",
                },
            },
            ...
            "should_not_retry": {
                "outbox_c": {
                    ...
                    "user_id": user_c_id,
                    "order_id": order_c,
                    "username": "user_c_fresh",
                    "reason": "too fresh (< 1 hour)",
                },
                "outbox_d": {
                    "id": outbox_d,
                    "user_id": user_d_id,
                    "order_id": order_d,
                    "username": "user_d_already_retried",
                ...
                "outbox_e": {
                    ...
                    "user_id": user_e_id,
                    "order_id": order_e,
                    "username": "user_e_no_nodes",
                    "reason": "no nodes (inactive sub)",
                },
            },
        }
...
''',
        ...
        await conn.execute(
            ...
            WHERE id = $2
        """,
            "encryption=none&type=tcp&security=tls",
            vnode_id_10,
        )
        await conn.execute(
            """
            ...
            WHERE id = $2
        """,
            "encryption=none&type=ws&path=/api",
            vnode_id_11,
        )
        # ========== СОЗДАЁМ ПОЛЬЗОВАТЕЛЕЙ ДЛЯ ПРОВЕРКИ SQL ФИЛЬТРОВ ==========
        ...
        """,
            ...
            pay_order_a,
            "uuid-sub-user-a",
            "b64-sub-active-a-valid-16chars",
        )
        ...
        """,
            ...
            pay_order_c,
            "uuid-sub-user-c",
            "b64-sub-expired-c-valid-16chars",
        )
        ...
        """,
            ...
            pay_order_d,
            "uuid-sub-user-d",
            "b64-sub-inactive-d-valid-16chars",
        )
        ...
        """,
            ...
            pay_order_e,
            "uuid-sub-user-e",
            "b64-sub-deleted-e-valid-16chars",
        )
        return {
            # Инфраструктура из arq_test_seed
            "plan_id": plan_id,
            ...
            "invalid_users": {
                "limit_exceeded": {
                    "user_id": user_b_id,
                    "b64_id": "b64-sub-limit-b-valid-16chars",
                    "reason": "traffic limit exceeded",
                },
                "expired": {
                    "user_id": user_c_id,
                    "b64_id": "b64-sub-expired-c-valid-16chars",
                    "reason": "subscription expired",
                },
                "inactive": {
                    "user_id": user_d_id,
                    "b64_id": "b64-sub-inactive-d-valid-16chars",
                    "reason": "subscription inactive",
                },
                "deleted": {
                    "user_id": user_e_id,
                    "b64_id": "b64-sub-deleted-e-valid-16chars",
                    "reason": "user deleted",
                },
            },
        }
...
async def metrics_collector_seed(db_pool, arq_test_seed, real_parser_scripts):
    ...
    async with db_pool.acquire() as conn:
        ...
        await conn.execute(
            ...
            WHERE id = (
                SELECT np.tmp_id FROM nodes_protocols np
                WHERE np.id = $2
            )
        """,
        ...
        metrics_plan_id = await conn.fetchval(
            ...
            RETURNING id
        """,
            "Metrics Traffic Test Plan",
            "Plan for traffic limit testing",
            True,
            98,
        )
        ...
        return {
            ...
            "should_block": {
                "user_a": {
                    "user_id": user_a_id,
                    "order_id": order_a,
                    "username": "user_a_should_block",
                    "initial_traffic": 500,
                },
                "user_b": {
                    "user_id": user_b_id,
                    "order_id": order_b,
                    "username": "user_b_should_block",
                    "initial_traffic": 800,
                },
            },
            ...
            "should_not_block": {
                "user_c": {
                    "user_id": user_c_id,
                    "order_id": order_c,
                    "username": "user_c_inactive_sub",
                    "reason": "inactive subscription",
                },
                "user_d": {
                    "user_id": user_d_id,
                    "order_id": order_d,
                    "username": "user_d_deleted",
                    "reason": "user deleted",
                },
                "user_e": {
                    "user_id": user_e_id,
                    "order_id": order_e,
                    "username": "user_e_already_limited",
                    "reason": "already limited",
                },
                "user_f": {
                    "user_id": user_f_id,
                    "order_id": order_f,
                    "username": "user_f_within_limit",
                    "reason": "within limit",
                },
                "user_g": {
                    "user_id": user_g_id,
                    "order_id": order_g,
                    "username": "user_g_expired_sub",
        ...
        return {
            ...
            "should_revoke": {
                "user_a": {
                    "user_id": user_a_id,
                    "order_id": order_a,
                    "username": "user_a_should_revoke",
                    "uuid": "uuid-revoke-user-a",
                },
                "user_b": {
                    "user_id": user_b_id,
                    "order_id": order_b,
                    "username": "user_b_should_revoke",
                    "uuid": "uuid-revoke-user-b",
                },
            },
            ...
            "should_not_revoke": {
                "user_c": {
                    "user_id": user_c_id,
                    "order_id": order_c,
                    "username": "user_c_already_inactive",
                    "reason": "already inactive",
                },
                "user_d": {
                    "user_id": user_d_id,
                    "order_id": order_d,
                    "username": "user_d_not_expired",
...
async def infinite_flags_test_seed(db_pool, arq_test_seed):
    ...
    async with db_pool.acquire() as conn:
        ...
        """,
            user_inf_traffic_id,
            plan_id,
            pay_order_inf_traffic,
            "uuid-inf-traffic-expired",
            "b64-inf-traffic-exp",
        )
        ...
        """,
            user_inf_expire_id,
            plan_id,
            pay_order_inf_expire,
            "uuid-inf-expire-overlimit",
            "b64-inf-exp-overlim",
        )
...
async def pointed_bulk_seed(db_pool, db_seed):
    ...
    async with db_pool.acquire() as conn:
        ...
        plan_id = await conn.fetchval(
            ...
            RETURNING id
        """,
            "Pointed Bulk Test Plan",
            "Plan for testing pointed bulk",
            True,
            1,
        )
        # Создаём оффер для плана
        ...
        """,
            ...
            "10.0.0.101",
            8101,
            "pointed-node-inactive",
            "Pointed Inactive Node",
            False,
        )
        # 3. Используем существующий шаблон протокола из seed_data
        tmp_id = await conn.fetchval("""
