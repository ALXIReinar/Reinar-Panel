...
[Unit]
Description=Sing-box Shadowsocks TLS HttpUpgrade Node (TMP_ID: ${TMP_ID})
...
curl -s -X POST "$PANEL_CALLBACK_URL" \
     ...
     -d '{
           ...
           "status": "installed",
           "node_type": "shadowsocks_tls_httpupgrade",
           "constant_node_data_obj": {
              "node_method": "'$SS_METHOD'"
           }
         }'

echo "Shadowsocks-tls-httpupgrade node $TMP_ID installed successfully."