...
WS_PATH="/ws-$(openssl rand -hex 4)-trojan"
...
CONFIG_PATH="$CONFIG_DIR/trojan-tls-ws-${NODE_PROTO_ID}.json"
cat <<EOF > "$CONFIG_PATH"
{
  "log": {
      "level": "warn"
...
[Unit]
Description=Sing-box Trojan WS TLS Node (NODE_PROTO_ID: ${NODE_PROTO_ID})
