...
@pytest.mark.asyncio
@pytest.mark.db
async def test_execute_bulk_add_user_unified(use_real_core, mock_xtlsapi, request, protocol_templates, is_real_mode):
...
async def test_execute_bulk_delete_user_unified(
    use_real_core, mock_xtlsapi, request, get_script_from_template, is_real_mode
):
...
@pytest.mark.asyncio
@pytest.mark.db
async def test_execute_bulk_add_unified(use_real_core, mock_xtlsapi, request, get_script_from_template, is_real_mode):
...
async def test_empty_script():
    ...
    success, message = await HotReloadExecutor.execute_action_script(
        script="", lib_names=None, user_obj=[{}], node_ip="127.0.0.1", core_api_port=10085, action="user_core_operation"
    )
    assert success is False
