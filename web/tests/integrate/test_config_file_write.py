...
async def vnode_with_template(db_pool, virtual_node_seed):
    ...
    async with db_pool.acquire() as conn:
        ...
        await conn.execute(
            "UPDATE nodes_protocols SET config_path = $1 WHERE id = $2", "/etc/xray/config.json", vnode_id
        )
...
class TestConfigFileWriteSuccess:
    ...
    async def test_write_config_success(self, client, vnode_with_template, db_pool):
        ...
        client.app.state.cmd_center_aiohttp = FakeAiohttpSession(
            json_data={
                "success": True,
                "config_link": "vless://test-uuid@example.com:443?encryption=none&security=reality#TestNode",
            }
        )
        ...
        response = await client.put(
            ...
            json={"node_proto_id": vnode_id, "file_content": config_content, "flatten_json_users_key": None},
        )
    ...
    async def test_write_config_updates_config_link_in_db(self, client, vnode_with_template, db_pool):
        ...
        client.app.state.cmd_center_aiohttp = FakeAiohttpSession(
            json_data={
                "success": True,
                "config_link": "vless://updated-uuid@example.com:443?encryption=none#UpdatedNode",
            }
        )
        ...
        async with db_pool.acquire() as conn:
            old_link = await conn.fetchval("SELECT config_link FROM nodes_protocols WHERE id = $1", vnode_id)
        ...
        async with db_pool.acquire() as conn:
            new_link = await conn.fetchval("SELECT config_link FROM nodes_protocols WHERE id = $1", vnode_id)
    ...
    async def test_write_config_with_flatten_key(self, client, vnode_with_template, db_pool):
        ...
        client.app.state.cmd_center_aiohttp = FakeAiohttpSession(
            json_data={
                "success": True,
                "config_link": "vless://flatten-test@example.com:443?encryption=none#FlattenTest",
            }
        )
        ...
        response = await client.put(
            ...
            json={
                ...
                "flatten_json_users_key": [
                    ...
                    "settings",
                    "clients",
                ],  # Список строк, а не строка с точками
            },
        )
...
class TestConfigFileWriteErrors:
    ...
    async def test_write_config_no_config_path(self, client, virtual_node_seed, db_pool):
        ...
        async with db_pool.acquire() as conn:
            await conn.execute("UPDATE nodes_protocols SET config_path = NULL WHERE id = $1", vnode_id)

        response = await client.put(
            "/api/v1/private/cmd_center/config_file/write",
            json={"node_proto_id": vnode_id, "file_content": '{"test": "config"}', "flatten_json_users_key": None},
        )
    ...
    async def test_write_config_node_error(self, client, vnode_with_template, db_pool):
        ...
        client.app.state.cmd_center_aiohttp = FakeAiohttpSession(json_data={"error": "Permission denied"}, status=403)
    ...
    async def test_write_config_link_generation_failed(self, client, vnode_with_template, db_pool):
        ...
        client.app.state.cmd_center_aiohttp = FakeAiohttpSession(
            json_data={"success": False, "error": "Url конфиг-ссылка не указана в шаблоне"}, status=400
        )
