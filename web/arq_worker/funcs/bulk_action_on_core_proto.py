from aiohttp import ClientResponseError, ClientSession  # noqa: I001
...
async def bulk_action_users_by_node(
    ...
    json2config_script: str,
    config2json_script: str,
    conf_converter_libs: str,
    current_attempt=1,
    db: PgSql = None,
    arq: ArqRedis = None,
    ...
    log_event(
        f'\033[31m[ARQ Bulk Action]\033[0m Бульк \033[33m{CoreProtoActions.id2name[operation]}\033[0m юзеров из конфиг-файла ядра | action: \033[33m{operation}\033[0m; raw_users_len: \033[35m{len(users)}\033[0m; node_proto_id: \033[33m{node_proto_id}\033[0m; private_ip: \033[33m{private_ip}\033[0m; api_port: \033[35m{api_port}\033[0m'
    )
    ...
    if not vpn_like_users:
        log_event(
            f'\033[31m[ARQ Bulk Action]\033[0m Не удалось создать ни одного впн-пользователя для ядра | node_proto_id: \033[31m{node_proto_id}\033[0m; action: \033[33m{operation}\033[0m',
            level='CRITICAL',
        )
    ...
    try:
        ...
        log_event(
            f'\033[31m[ARQ Bulk Action]\033[0m Юзеры удалены из конфиг-файла ядра | action: \033[33m{operation}\033[0m; users_len: \033[35m{len(users)}\033[0m; node_proto_id: \033[33m{node_proto_id}\033[0m; private_ip: \033[33m{private_ip}\033[0m; api_port: \033[35m{api_port}\033[0m'
        )
        return {'success': True, 'message': 'Пользователи удалены из инстанса ядра'}
    except ClientResponseError as err:
        if err.status == 422:
            log_event(
                f'\033[31m[ARQ Bulk Delete]\033[0m Ошибка валидации в Инстансе ядре. Неправильные | action: \033[33m{operation}\033[0m; node_proto_id: \033[33m{node_proto_id}\033[0m; error: {err}',
                level='WARNING',
            )
            return {
                'success': False,
                'message': 'Не удалось прокинуть Бульк вставку пользователей в ядро. 422 от нод-клиента',
            }
        log_event(
            f'\033[31m[ARQ Bulk Action]\033[0m HTTP ошибка | action: \033[33m{operation}\033[0m; node_proto_id: \033[33m{node_proto_id}\033[0m; operation: \033[36mbulk-add\033[0m; status: \033[31m{err.status}\033[0m',
            level='ERROR',
        )

    except Exception as e:
        log_event(
            f'\033[31m[ARQ Bulk Action]\033[0m Ошибка запроса на \033[33m{CoreProtoActions.id2name[operation]}\033[0m. Ретрай | action: \033[33m{operation}\033[0m; node_proto_id: \033[33m{node_proto_id}\033[0m; users_len: {len(users)}; error: \033[36m{e}\033[0m',
            level='ERROR',
        )
    ...
    if current_attempt < max_tries:
        log_event(
            f'\033[31m[ARQ Bulk Action]\033[0m Планируем retry | action: \033[33m{operation}\033[0m; attempt: \033[33m{current_attempt + 1}/{max_tries}\033[0m; users_len: \033[33m{len(users)}\033[0m; node_proto_id: \033[36m{node_proto_id}\033[0m',
            level='WARNING',
        )
        ...
        log_event(
            f'\033[31m[ARQ Bulk Action]\033[0m Бульк операция. Превышено количество попыток | action: \033[33m{operation}\033[0m; max_tries: {max_tries}; node_proto_id: \033[31m{node_proto_id}\033[0m',
            level='ERROR',
        )
