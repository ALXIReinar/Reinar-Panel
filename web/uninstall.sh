...
# Удаляем образы созданные для этого проекта (web-service, sub-service, arq-sub-worker)
docker images | grep -E 'web-(web-service|sub-service|arq-sub-worker)' | awk '{print $3}' | xargs -r docker rmi 2>/dev/null || true
