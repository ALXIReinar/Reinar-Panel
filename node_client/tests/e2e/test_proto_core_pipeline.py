...
from node_client.api.proto_core.write_behind_caching_file import flatten_key2value  # noqa: F401


# ========== Helper функции ==========


def create_test_superuser(user_uuid: str = None, user_sub_id: str = "42", **constant_fields) -> dict:
    """
    ...
    superuser = {"user_uuid": user_uuid, "user_sub_id": user_sub_id, **constant_fields}
    return superuser
...
@pytest.mark.db
@pytest.mark.slow
async def test_add_user_without_api_script_only_file(e2e_client, e2e_config_path, e2e_buffer, get_script_from_template):
    """
    ...
    with patch('asyncio.create_subprocess_shell', mock_subprocess):
        ...
        test_user = create_test_superuser(
            user_uuid="e2e-test-uuid-no-script", user_sub_id="test_sub_42", flow="xtls-rprx-vision", level=0
        )
...
async def test_delete_user_without_api_script_only_file(
    e2e_client, e2e_config_path, e2e_buffer, get_script_from_template
):
    """
    ...
    test_user = create_test_superuser(
        user_uuid="e2e-delete-uuid", user_sub_id="delete_sub_42", flow="xtls-rprx-vision", level=0
    )
...
async def test_add_user_with_api_script_success_no_reload(
    e2e_client, e2e_config_path, e2e_buffer, get_script_from_template, mock_xtlsapi_e2e
):
    """
    ...
    with patch('asyncio.create_subprocess_shell', mock_subprocess):
        ...
        # Создаём суперобъект
        test_user = create_test_superuser(
            user_uuid="e2e-hot-reload-success", user_sub_id="hot_reload_sub_42", flow="xtls-rprx-vision", level=0
        )
        request_body = create_e2e_request_body(
            node_proto_id=1,
            users=[test_user],
...
"""
    ...
    with patch('asyncio.create_subprocess_shell', mock_subprocess):
        # Создаём суперобъект
        test_user = create_test_superuser(
            user_uuid="e2e-hot-reload-fail", user_sub_id="hot_reload_fail_sub", flow="xtls-rprx-vision", level=0
        )
        request_body = create_e2e_request_body(
            node_proto_id=1,
            users=[test_user],
...
async def test_bulk_add_users_with_api_script_unlimit_flush(
    ...
    users_to_add = [
        create_test_superuser(user_uuid=f"bulk-uuid-{i}", user_sub_id=f"bulk_sub_{i}", flow="xtls-rprx-vision", level=0)
        for i in range(10)
    ]
    request_body = create_e2e_request_body(
        node_proto_id=1,
...
@pytest.mark.db
@pytest.mark.slow
async def test_bulk_delete_users_without_api_script(e2e_client, e2e_config_path, e2e_buffer, get_script_from_template):
    """
    ...
    users_to_add = [
        create_test_superuser(
            user_uuid=f"bulk-del-{i}", user_sub_id=f"bulk_del_sub_{i}", flow="xtls-rprx-vision", level=0
        )
        for i in range(5)
    ]
...
async def test_e2e_add_user_with_hot_reload_and_verify_metrics(
    ...
    if use_real_core:
        ...
        core_ip, core_port = "127.0.0.1", 10085  # noqa: F841
    ...
    test_user = create_test_superuser(
        user_uuid=test_user_uuid, user_sub_id=test_user_sub_id, flow="xtls-rprx-vision", level=0
    )
    add_request = create_e2e_request_body(
        node_proto_id=1,
        users=[test_user],
...
async def test_e2e_bulk_operations_with_verification(
    ...
    users_to_add = [
        create_test_superuser(
            user_uuid=f"bulk-verify-{i}", user_sub_id=f"bulk_verify_{i}", flow="xtls-rprx-vision", level=0
        )
        for i in range(5)
    ]
    user_injectors = create_default_user_injectors()
