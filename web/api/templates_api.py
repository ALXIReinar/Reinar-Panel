...
async def get_all_templates(params: GetTmpSchema, request: Request, db: PgSqlDep, _: JWTCookieDep):
    ...
    log_event(
        f'Отдали список шаблонов | tmp_len: \033[32m{len(templates)}\033[0m; admin_id: \033[31m{request.state.admin_id}\033[0m',
        request=request,
    )
...
@router.get('/{tmp_id}')
async def get_tmp_by_id(tmp_id: int, request: Request, db: PgSqlDep, _: JWTCookieDep):
    """Получить шаблон по ID с привязанными spec параметрами"""
    result = await db.proto_templates.get_by_id(tmp_id)
    if not result:
        log_event(
            f'Шаблон не найден | tmp_id: \033[31m{tmp_id}\033[0m; admin_id: \033[31m{request.state.admin_id}\033[0m',
            request=request,
            level='WARNING',
        )
        raise HTTPException(status_code=404, detail='Шаблон не найден')
    log_event(
        f'Отдали шаблон | tmp_id: \033[32m{tmp_id}\033[0m; admin_id: \033[31m{request.state.admin_id}\033[0m',
        request=request,
    )
...
async def add_template(body: AddTmpSchema, request: Request, db: PgSqlDep, _: JWTCookieDep):
    ...
    log_event(
        f'Создание шаблона | title: \033[35m{body.title}\033[0m; admin_id: \033[31m{request.state.admin_id}\033[0m',
        request=request,
    )
    ...
    if status_code == 409:
        log_event(
            f'Конфликт при создании шаблона | title: \033[33m{body.title}\033[0m; admin_id: \033[31m{request.state.admin_id}\033[0m',
            request=request,
            level='WARNING',
        )
        raise HTTPException(status_code=409, detail={'success': False, 'message': message})
    log_event(
        f'Шаблон создан | tmp_id: \033[32m{template_id}\033[0m; admin_id: \033[32m{request.state.admin_id}\033[0m',
        request=request,
    )
...
async def update_template(tmp_id: int, body: UpdateTmpSchema, request: Request, db: PgSqlDep, _: JWTCookieDep):
    log_event(
        f'Обновление шаблона | tmp_id: \033[35m{tmp_id}\033[0m; admin_id: \033[31m{request.state.admin_id}\033[0m',
        request=request,
    )
    ...
    log_event(
        f'Шаблон обновлён | tmp_id: \033[32m{tmp_id}\033[0m; body: \033[37m{repr(body)}\033[0m; admin_id: \033[32m{request.state.admin_id}\033[0m',
        request=request,
    )
...
async def update_user_injectors(
    tmp_id: int, body: EditUserInjectorsSchema, db: PgSqlDep, request: Request, _: JWTCookieDep
):
    log_event(
        f'Изменение инжекторов шаблона | state_injectors_len: \033[33m{len(body.user_injectors)}\033[0m; tmp_id: \033[35m{tmp_id}\033[0m; admin_id: \033[31m{request.state.admin_id}\033[0m',
        request=request,
    )
    ...
    if not res:
        log_event(
            f'Не удалось обновить инжекторы шаблона. Не существует! | state_injectors_len: \033[35m{len(body.user_injectors)}\033[0m; tmp_id: \033[34m{tmp_id}\033[0m; admin_id: \033[31m{request.state.admin_id}\033[0m'
        )
        raise HTTPException(
            status_code=404, detail={'success': False, 'message': "Шаблон с таким tmp_id не существует"}
        )

    log_event(
        f'Обновили инжекторы шаблона! | state_injectors_len: \033[33m{len(body.user_injectors)}\033[0m; tmp_id: \033[36m{tmp_id}\033[0m; admin_id: \033[31m{request.state.admin_id}\033[0m',
        request=request,
    )
...
async def delete_template(tmp_id: int, request: Request, db: PgSqlDep, _: JWTCookieDep):
    ...
    log_event(
        f'Удаление шаблона | tmp_id: \033[35m{tmp_id}\033[0m; admin_id: \033[31m{request.state.admin_id}\033[0m',
        request=request,
    )
    ...
    if status_code == 409:
        log_event(
            f'Невозможно удалить шаблон (используется) | tmp_id: \033[33m{tmp_id}\033[0m; admin_id: \033[31m{request.state.admin_id}\033[0m',
            request=request,
            level='WARNING',
        )
        raise HTTPException(status_code=409, detail={'success': False, 'message': message})
    ...
    log_event(
        f'Шаблон удалён | tmp_id: \033[32m{tmp_id}\033[0m; admin_id: \033[32m{request.state.admin_id}\033[0m',
        request=request,
        level='WARNING',
    )
    return {'success': True, 'message': message}
