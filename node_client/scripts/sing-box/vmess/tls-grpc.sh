...
if [ -z "$CERT_PATH" ] || [ -z "$KEY_PATH" ] || [ -z "$DOMAIN" ]; then
    ...
    log "Использование: bash node_client/scripts/sing-box/vmess/tls-grpc.sh <cert_path> <key_path> <domain>"
    exit 1
fi
...
CONFIG_DIR="/etc/reinar/configs/sing-box/vmess"
...
[Unit]
Description=Sing-box Vmess TLS Grpc Node (NODE_PROTO_ID: ${NODE_PROTO_ID})
