...
@arq_dep
@pg_sql_dep
async def pointed_bulk_action(
    ctx: dict, outbox_event_ids: list[int], action: Literal['add', 'delete'], db: PgSql = None, arq: ArqRedis = None
):
    if not outbox_event_ids:
        return {'success': False, 'message': 'Нет оутбоксов для вставки пользователей!'}

    log_event(
        f'\033[33m[ARQ Pointer Actioner]\033[0m Точечная бульк операция на нодах | action: \033[32m{action}\033[0m',
        level='WARNING',
    )
    ...
    log_event(
        f'\033[33m[ARQ Pointer Actioner]\033[0m Статистика перед запуском | total_operations: \033[31m{users_to_action}\033[0m',
        level='WARNING',
    )
    ...
    for vnode in nodes_meta:
        if len(vnode['users']) > 0:
            log_event(
                f'\033[33m[ARQ Pointer Actioner]\033[0m Отправляем Бульк запрос на фоновое \033[31m{action}\033[0m пользователей в ядра | node_proto_id: \033[33m{vnode["node_proto_id"]}\033[0m'
            )
            ...
            log_event(
                f'\033[33m[Pointer Actioner]\033[0m Фоновая задача запущена, бульк-\033[31m{action}\033[0m | node_proto_id: \033[33m{vnode["node_proto_id"]}\033[0m',
                job_id=job.job_id,
            )
