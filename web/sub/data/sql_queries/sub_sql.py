...
class SubscriptionQueries:
    ...
    async def get_sub_links(self, b64_string: str):
        query_sub_meta = '''
        SELECT 
            us.sub_plan_id, sp.title, sp.description, sp.traffic_limit_day AS sub_plan_limit, u.id AS user_id, u.uuid AS user_uuid,
            u.traffic_used_day_mb, us.expire_date
        FROM users u
    ...
    async def get_core_proto_deps_by_user_id(
            self, user_id: int, user_uuid: str, tg_username: str, user_sub_id: int, operation: CoreProtoActions | int
    ):
        """
        ...
        '''
        return await self.conn.fetch(query, user_id, user_uuid, tg_username, user_sub_id, operation)
    ...
    async def get_and_lock_expired_subs_grouped_by_node(self):
        ...
        WITH deactivated_subs AS (
            ...
            RETURNING id AS user_sub_id, sub_plan_id, user_id, order_id
        ),
        ...
        expired_nodes_info AS (
            SELECT u.uuid, u.tg_username, ds.user_sub_id, vsp.node_proto_id, n.private_ip, n.api_port, np.metrics_port, pt.proto_python_lib,
        ...
        insert_outbox AS (
            INSERT INTO sub_nodes_outbox (user_uuid, tg_username, user_sub_id, operation, node_proto_id)
            SELECT uuid, tg_username, user_sub_id, $1, node_proto_id
    ...
    async def get_sub_nodes_for_bulk_action(self, users: list[dict]):
        user_sub_ids, sub_plan_ids, user_ids = zip(*tuple(u.values() for u in users))
        ...
        GROUP BY node_proto_id, private_ip, api_port, metrics_port, proto_python_lib, 
                 ...
                 api_bulk_delete_user_script, bulk_delete_script_custom_params
        '''
        return await self.conn.fetch(query, user_sub_ids, sub_plan_ids, user_ids)
    ...
    async def success_bulk_core_proto_users(self, node_proto_ids: list[int], user_sub_ids: list[int], action: CoreProtoActions | int):
        query = '''
    ...
    async def update_traffic(self, tg_usernames: list[str], traffic_add_mbs: list[int]):
        ...
        users_limited AS (
            UPDATE user_subs SET is_limited = true 
            FROM (
                SELECT it.user_id FROM increase_traffic it
                JOIN user_subs us ON us.user_id = it.user_id AND us.is_active = true 
                JOIN sub_plans sp ON sp.id = us.sub_plan_id
                WHERE it.traffic_used_day_mb > sp.traffic_limit_day AND us.is_limited = false
            ) AS limited_users
            WHERE user_subs.user_id = limited_users.user_id AND user_subs.is_active = true
            RETURNING user_subs.user_id, user_subs.id, user_subs.sub_plan_id
        )
        INSERT INTO sub_nodes_outbox (user_uuid, tg_username, user_sub_id, operation, node_proto_id)
    ...
    async def get_vnodes_by_outbox_events(self, outbox_ids: list[int]):
        ...
        WITH limited_nodes_info AS (
            SELECT u.uuid, u.tg_username, us.id AS user_sub_id, sno.node_proto_id, n.private_ip, n.api_port, np.metrics_port,
