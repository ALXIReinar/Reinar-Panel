...
async def add_tg_user(body: UserAddSchema, request: Request, db: PgSqlDep):
    ...
    log_event(
        f'\033[36m[Tg Routing]\033[0m Попытка создать пользователя | insert_res: \033[35m{insert_success}\033[0m; user_id: \033[33m{user_info.get("user_id")}\033[0m',
        request=request,
    )
    if body.return_data:
...
async def get_tg_user(tg_id: Annotated[int, Query()], request: Request, db: PgSqlDep):
    ...
    log_event(
        f'\033[36m[Tg Routing]\033[0m Отдали профиль инфо | user_id: \033[36m{user_info["id"]}\033[0m', request=request
    )
    return user_info
...
async def get_tg_user_subs(tg_id: Annotated[int, Query()], request: Request, db: PgSqlDep):
    ...
    log_event(
        f'\033[36m[Tg Routing]\033[0m Отдали подписки юзера | user_sub_len: \033[32m{len(user_subs)}\033[0m; tg_id: \033[31m{tg_id}\033[0m',
        request=request,
    )
...
async def get_shop_sub_plans(request: Request, db: PgSqlDep):
    ...
    log_event(
        f'\033[36m[Tg Routing]\033[0m Отдали тарифные планы | sub_plans_len: \033[32m{len(sub_plans)}\033[0m',
        request=request,
    )
