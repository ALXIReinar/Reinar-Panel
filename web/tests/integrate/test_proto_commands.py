...
async def protocol_with_commands_seed(client: AsyncClient, db_seed, proto_template_seed, db_pool):
    ...
    response = await client.post(
        "/api/v1/private/protocols/create", json={"name": "TestProtocol", "tmp_id": proto_template_seed["tmp_id"]}
    )
...
async def test_get_protocol_commands_empty(client: AsyncClient, db_seed, proto_template_seed):
    ...
    response = await client.post(
        "/api/v1/private/protocols/create", json={"name": "EmptyProtocol", "tmp_id": proto_template_seed["tmp_id"]}
    )
    proto_id = response.json()["proto_id"]
...
async def test_bulk_insert_commands_success(client: AsyncClient, db_seed, proto_template_seed):
    ...
    # Создаём протокол
    response = await client.post(
        "/api/v1/private/protocols/create", json={"name": "BulkInsertProto", "tmp_id": proto_template_seed["tmp_id"]}
    )
    proto_id = response.json()["proto_id"]
    ...
    response = await client.post(
        "/api/v1/private/protocol-commands/bulk/add", json={"proto_id": proto_id, "commands": commands}
    )
...
async def test_bulk_insert_single_command(client: AsyncClient, db_seed, proto_template_seed):
    ...
    # Создаём протокол
    response = await client.post(
        "/api/v1/private/protocols/create", json={"name": "SingleCmdProto", "tmp_id": proto_template_seed["tmp_id"]}
    )
    proto_id = response.json()["proto_id"]
...
async def test_bulk_insert_invalid_proto_id(client: AsyncClient, db_seed):
    ...
    response = await client.post(
        "/api/v1/private/protocol-commands/bulk/add", json={"proto_id": 9999, "commands": commands}
    )
...
async def test_bulk_update_commands_success(client: AsyncClient, protocol_with_commands_seed):
    ...
    response = await client.put(
        "/api/v1/private/protocol-commands/bulk/update", json={"proto_id": proto_id, "commands": commands}
    )
