...
if [ -z "$TMP_ID" ] || [ -z "$CERT_PATH" ] || [ -z "$KEY_PATH" ]; then
    ...
    echo "Использование: bash trojan-httpupgrade-tls-install.sh <tmp_id> <cert_path> <key_path> <domain>"
    exit 1
fi
  ...
  "inbounds": [
    {
      ...
      "streamSettings": {
        ...
        "httpupgradeSettings": {
          "path": "/http-$HTTP_PATH-trojan-tls",
          "host": "$DOMAIN"
        }
      },
      "tag": "trojan-inbound"
...
[Unit]
Description=Xray Trojan-HTTPUpgrade-TLS Node (TMP_ID: ${TMP_ID})
...
curl -s -X POST "$PANEL_CALLBACK_URL" \
     ...
     -d '{
           ...
           "status": "installed",
           "node_type": "trojan_httpupgrade_tls"
         }'

echo "Trojan-HTTPUpgrade-TLS нода $TMP_ID успешно установлена."