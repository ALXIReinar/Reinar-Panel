...
if [ -z "$CERT_PATH" ] || [ -z "$KEY_PATH" ] || [ -z "$SNI_DOMAIN" ]; then
    ...
    log "Использование: bash node_client/scripts/sing-box/tuicv5/tls.sh <cert_path> <key_path> <domain>"
    exit 1
fi
...
CONFIG_DIR="/etc/reinar/configs/sing-box/tuicv5"
...
CONFIG_PATH="$CONFIG_DIR/tuicv5-tls-${NODE_PROTO_ID}.json"
cat <<EOF > "$CONFIG_PATH"
{
  "log": {
    "level": "warn"
...
SERVICE_NAME="reinar-tuicv5-${NODE_PROTO_ID}"
SERVICE_PATH="/etc/systemd/system/${SERVICE_NAME}.service"
cat <<EOF > "$SERVICE_PATH"
[Unit]
Description=Sing-box TUIC v5 Node (NODE_PROTO_ID: ${NODE_PROTO_ID})
