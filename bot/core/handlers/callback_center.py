...
async def callback_factory(call: CallbackQuery, redis: Redis, state: FSMContext, aio_http: SubServiceConn):
    ...
    # Раздел Подписок магазина
        ...
        await ShopSubscriptions.shop_subscriptions_slider(call.message, redis, aio_http)
        ...
        text, kb = await ShopSubscriptions.build_shop_plans_slider_msg(sub_plan_idx, call.message, redis)
        ...
        text, kb = await ShopSubscriptions.give_issued_payment(sub_plan_idx, offer_idx, call.message, redis, aio_http)
    ...
    # Раздел подписок пользователя
        ...
        text, kb = await UserSubscriptions.give_issued_payment(user_sub_idx, offer_idx, call.message, state, aio_http)
        # 1. В случае пропажи подписок в state слайдер отобразится снова
        # 2. Сообщение об ошибке формирования заказа
        if text:
            await call.message.edit_text(text, reply_markup=kb)
