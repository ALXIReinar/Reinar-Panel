...
class TestBulkAddSuccess:
    ...
    async def test_bulk_add_creates_users(self, client, db_pool):
        ...
        users_data = [{"tg_username": "new_user_1", "tg_id": 3000001}, {"tg_username": "new_user_2", "tg_id": 3000002}]

        response = await client.post("/api/v1/private/users/bulk/add", json={"users": users_data})
    ...
    async def test_bulk_add_returns_created_users(self, client, db_pool):
        ...
        users_data = [{"tg_username": "return_test_user", "tg_id": 3000200}]
...
class TestBulkAddValidation:
    ...
    async def test_bulk_add_empty_users_list(self, client):
        ...
        response = await client.post("/api/v1/private/users/bulk/add", json={"users": []})
    ...
    async def test_bulk_add_duplicate_tg_username_in_request(self, client, db_pool):
        ...
        async with db_pool.acquire() as conn:
            count = await conn.fetchval("SELECT COUNT(*) FROM users WHERE tg_username = $1", "duplicate_user")
