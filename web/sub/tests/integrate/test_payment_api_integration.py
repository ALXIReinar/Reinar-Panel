...
def create_valid_webhook_payload_dict(
    order_id: int,
    user_id: int,
    sub_plan_id: int,
    offer_id: int,
...
class TestWebhookProcessing:
    ...
    async def test_webhook_enqueues_arq_job(self, test_payment_client, payment_seed, db_pool):
        ...
        assert len(call_args[0]) == 5, (
            f"Ожидалось 5 параметров (task_name, user_uuid, user_sub_id, sub_nodes, "
            f"operation), получено: {len(call_args[0])}"
        )
        ...
        assert operation in (1, 'add'), f"operation должна быть ADD (1 или 'add'), получено: {operation}"
        # Assert 7: sub_nodes - непустой список
        assert isinstance(sub_nodes, list) and len(sub_nodes) > 0, (
            f"sub_nodes должен быть непустым списком, получено: {type(sub_nodes)} "
            f"с длиной {len(sub_nodes) if isinstance(sub_nodes, list) else 'N/A'}"
        )
        ...
        for idx, node in enumerate(sub_nodes):
            ...
            assert isinstance(node['private_ip'], str) and len(node['private_ip']) > 0, (
                "private_ip должен быть непустой строкой"
            )
            assert isinstance(node['api_port'], int) and node['api_port'] > 0, f"api_port должен быть положительным int"  # noqa: F541
            assert isinstance(node['proto_python_lib'], str), f"proto_python_lib должен быть строкой"  # noqa: F541
            assert isinstance(node['user_injectors'], list), f"user_injectors должен быть списком (может быть пустым)"  # noqa: F541
            assert isinstance(node['event_id'], int) and node['event_id'] > 0, (
                "event_id должен быть положительным int (outbox record id)"
            )
        ...
        async with db_pool.acquire() as conn:
            ...
            assert outbox_node_ids == expected_node_ids_set, (
                f"outbox записи созданы не для всех нод. "
                f"Ожидалось: {expected_node_ids_set}, получено: {outbox_node_ids}"
            )
            # Проверяем что outbox записи содержат корректные данные
            for rec in outbox_records:
                assert rec['user_uuid'] == user_uuid, f"outbox.user_uuid не совпадает с переданным в ARQ"  # noqa: F541
                assert rec['user_sub_id'] == user_sub_id, f"outbox.user_sub_id не совпадает с переданным в ARQ"  # noqa: F541
                assert rec['operation'] == 1, f"outbox.operation должна быть ADD (1), получено: {rec['operation']}"
