...
from aiohttp import ClientSession, ClientResponseError  # noqa: F401
...
async def revoke_sub_plan_by_expire(ctx: dict, db: PgSql = None, arq: ArqRedis = None):
    log_event(
        '\033[31m[ARQ Sub Revoke]\033[0m Срок действия подписки истёк. Крона по удалению пользователей из ядер протоколов'
    )
    ...
    log_event(
        f'\033[31m[ARQ Sub Revoke]\033[0m Крона по удалению пользователей из ядер протоколов | total_deletes: \033[31m{users_to_delete}\033[0m'
    )
    ...
    async def enqueue_delete(vnode):
        async with sem:
            ...
            log_event(
                f'\033[31m[ARQ Sub Revoke]\033[0m Отправляем Бульк запрос на фоновое удаление пользователей из ядра | node_proto_id: \033[33m{vnode["node_proto_id"]}\033[0m'
            )
            ...
            log_event(
                f'\033[31m[ARQ Sub Revoke]\033[0m Фоновая задача запущена | node_proto_id: \033[33m{vnode["node_proto_id"]}\033[0m',
                job_id=job.job_id,
            )
