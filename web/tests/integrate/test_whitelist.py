...
async def test_get_whitelist_empty_cache_loads_from_db(client: AsyncClient, db_seed, flush_redis, db_pool):
    ...
    async with db_pool.acquire() as conn:
        await conn.execute(
            "INSERT INTO whitelist_commands (command, is_active) VALUES ($1, $2), ($3, $4)", "ls", True, "cat", False
        )
...
async def test_get_whitelist_from_cache(client: AsyncClient, db_seed, flush_redis, db_pool):
    ...
    # Создаём команды в БД
    async with db_pool.acquire() as conn:
        await conn.execute("INSERT INTO whitelist_commands (command, is_active) VALUES ($1, $2)", "pwd", True)
...
async def test_get_whitelist_data_structure(client: AsyncClient, db_seed, db_pool):
    ...
    async with db_pool.acquire() as conn:
        await conn.execute("INSERT INTO whitelist_commands (command, is_active) VALUES ($1, $2)", "echo", True)
...
async def test_bulk_update_activate_commands_flushes_redis(client: AsyncClient, db_seed, flush_redis, db_pool):
    ...
    async with db_pool.acquire() as conn:
        cmd1_id = await conn.fetchval(
            "INSERT INTO whitelist_commands (command, is_active) VALUES ($1, $2) RETURNING id", "grep", False
        )
        cmd2_id = await conn.fetchval(
            "INSERT INTO whitelist_commands (command, is_active) VALUES ($1, $2) RETURNING id", "find", False
        )
    ...
    response = await client.put(
        "/api/v1/private/whitelist/bulk/update", json={"set_as_active": [cmd1_id, cmd2_id], "set_as_inactive": []}
    )
...
async def test_bulk_update_deactivate_commands_flushes_redis(client: AsyncClient, db_seed, flush_redis, db_pool):
    ...
    async with db_pool.acquire() as conn:
        cmd1_id = await conn.fetchval(
            "INSERT INTO whitelist_commands (command, is_active) VALUES ($1, $2) RETURNING id", "rm", True
        )
        cmd2_id = await conn.fetchval(
            "INSERT INTO whitelist_commands (command, is_active) VALUES ($1, $2) RETURNING id", "cp", True
        )
    ...
    response = await client.put(
        "/api/v1/private/whitelist/bulk/update", json={"set_as_active": [], "set_as_inactive": [cmd1_id, cmd2_id]}
    )
...
async def test_bulk_update_combined_operations(client: AsyncClient, db_seed, flush_redis, db_pool):
    ...
    async with db_pool.acquire() as conn:
        active_id = await conn.fetchval(
            "INSERT INTO whitelist_commands (command, is_active) VALUES ($1, $2) RETURNING id", "mv", True
        )
        inactive_id = await conn.fetchval(
            "INSERT INTO whitelist_commands (command, is_active) VALUES ($1, $2) RETURNING id", "touch", False
        )
    ...
    response = await client.put(
        "/api/v1/private/whitelist/bulk/update", json={"set_as_active": [inactive_id], "set_as_inactive": [active_id]}
    )
...
async def test_bulk_update_deactivate_all_disables_whitelist_mode(client: AsyncClient, db_seed, flush_redis, db_pool):
    ...
    async with db_pool.acquire() as conn:
        cmd_id = await conn.fetchval(
            "INSERT INTO whitelist_commands (command, is_active) VALUES ($1, $2) RETURNING id", "whoami", True
        )
    ...
    response = await client.put(
        "/api/v1/private/whitelist/bulk/update", json={"set_as_active": [], "set_as_inactive": [cmd_id]}
    )
    ...
    async with db_pool.acquire() as conn:
        active_count = await conn.fetchval("SELECT COUNT(*) FROM whitelist_commands WHERE is_active = true")
...
async def test_bulk_insert_commands_flushes_redis(client: AsyncClient, db_seed, flush_redis):
    ...
    await client.get("/api/v1/private/whitelist/all")
    redis: Redis = flush_redis
    cached_before = await redis.get("wh_list_commands")  # noqa: F841
    # Кэш может быть пустым или содержать пустой список

    # Добавляем команды
    response = await client.post("/api/v1/private/whitelist/bulk/add", json={"commands": ["sed", "awk", "sort"]})
...
async def test_bulk_add_partial_with_duplicates(client: AsyncClient, db_seed, db_pool):
    ...
    async with db_pool.acquire() as conn:
        await conn.execute("INSERT INTO whitelist_commands (command, is_active) VALUES ($1, $2)", "head", True)
...
async def test_bulk_add_first_command_enables_whitelist_mode(client: AsyncClient, db_seed, flush_redis, db_pool):
    ...
    response = await client.post("/api/v1/private/whitelist/bulk/add", json={"commands": ["hostname"]})
...
async def test_bulk_delete_commands_flushes_redis(client: AsyncClient, db_seed, flush_redis, db_pool):
    ...
    async with db_pool.acquire() as conn:
        cmd1_id = await conn.fetchval(
            "INSERT INTO whitelist_commands (command, is_active) VALUES ($1, $2) RETURNING id", "df", True
        )
        cmd2_id = await conn.fetchval(
            "INSERT INTO whitelist_commands (command, is_active) VALUES ($1, $2) RETURNING id", "du", True
        )
    ...
    # Удаляем команды
    response = await client.request("DELETE", "/api/v1/private/whitelist/bulk/delete", json={"ids": [cmd1_id, cmd2_id]})
...
async def test_bulk_delete_all_disables_whitelist_mode(client: AsyncClient, db_seed, flush_redis, db_pool):
    ...
    async with db_pool.acquire() as conn:
        cmd1_id = await conn.fetchval(
            "INSERT INTO whitelist_commands (command, is_active) VALUES ($1, $2) RETURNING id", "uptime", True
        )
        cmd2_id = await conn.fetchval(
            "INSERT INTO whitelist_commands (command, is_active) VALUES ($1, $2) RETURNING id", "date", True
        )
...
async def test_bulk_delete_nonexistent_ids_succeeds(client: AsyncClient, db_seed, db_pool):
    ...
    response = await client.request("DELETE", "/api/v1/private/whitelist/bulk/delete", json={"ids": [99999, 88888]})
