...
from web.sub.config_dir import config as cfg  # noqa: E402
from web.sub.anything import VnodeRegStatuses  # noqa: E402
...
async def real_parser_scripts(db_pool):
    ...
    async with db_pool.acquire() as conn:
        ...
        """)
        if not parsers:
            raise RuntimeError("Не найдено парсеров метрик в proto_templates! Запустите: python -m web.db.seed_data")
...
async def sub_api_seed(db_pool, sub_infrastructure_seed):
    ...
    async with db_pool.acquire() as conn:
        ...
        if not template_data or not template_data['sub_prepare_script']:
            raise RuntimeError(
                f"Шаблон для vnode_id={vnode_id_10} не имеет sub_prepare_script! Запустите: python -m web.db.seed_data"
            )
        ...
        config_link_10 = render_config_link_for_test(url_tmp=url_tmp, node_config_json=mock_config)
        # Рендерим config_link для vnode_11
        config_link_11 = render_config_link_for_test(url_tmp=url_tmp, node_config_json=mock_config)
        ...
        """,
            user_a_id,
            sub_infrastructure_seed['offer_id'],
        )
...
async def sub_prepare_infrastructure(db_pool):
    ...
    async with db_pool.acquire() as conn:
        ...
        """)  # noqa: W291
        if not templates:
            raise RuntimeError("Не найдено шаблонов с sub_prepare_script! Запустите: python -m web.db.seed_data")
