...
CONFIG_DIR="/etc/reinar/configs/sing-box/ss"
...
HTTPUP_PATH="/httpup-$(openssl rand -hex 4)-ss"
...
CONFIG_PATH="$CONFIG_DIR/ss-tls-httpup-${NODE_PROTO_ID}.json"
cat <<EOF > "$CONFIG_PATH"
{
  "log": {
      "level": "warn"
...
[Unit]
Description=Sing-box Shadowsocks TLS HttpUpgrade Node (NODE_PROTO_ID: ${NODE_PROTO_ID})
